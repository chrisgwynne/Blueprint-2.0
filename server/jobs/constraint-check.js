 ...
 *
 * Wraps a scheduled job with a pre-run constraint check. If constraints
 * are active, the job is delayed (not skipped) and logged.
 */
import crypto from 'crypto';
import db from '../db/db.js';

// Agent → primary metric types this agent analyses.
// Used to decide whether to delay an agent while its primary metrics have
// too many actions in measurement windows.
const AGENT_METRIC_MAP = {
...
// Agents whose analysis depends on data that refreshes at a specific time.
// We delay if the primary data source hasn't been synced within threshold.
const AGENT_DATA_FRESHNESS = {
 ...
 * @returns {{ allowed: boolean, reason: string|null, delay_until: string|null, constraints: object }}
 */
export function checkAgentConstraints(agentId, businessId) {
  const constraints = { in_flight_count: 0, data_stale: false };
  ...
  if (freshness) {
    ...
    ).get(businessId, freshness.connector);
 ...
 * Pass-through — imports and calls the real runAgent only if constraints pass.
 */
export async function runAgentWithConstraints(agentId, businessId, trigger, triggerId, opts) {
 ...
 */
export function applySmartSpacing(completedTask) {
  ...
  // Get the new do_not_touch_until from action_memory
 ...
 */
export function listRecentSchedulerLog(limit = 50) {
  return db.prepare(`
