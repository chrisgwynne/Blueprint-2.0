...
async function fetchViaAPI(credentials: Creds): Promise<KirbyApiResponse | null> {
  ...
  try {
    ...
    return await res.json() as KirbyApiResponse;
  } catch {
    return null;
  }
}

async function fetchViaFilesystem(contentPath: string | undefined): Promise<{ pages: KirbyPage[] } | null> {
  if (!contentPath) return null;
  const { readdir, readFile, stat } = await import('fs/promises');
  const { join } = await import('path');

  const pages: KirbyPage[] = [];

  async function walk(dir: string, relativeId = ''): Promise<void> {
...
function normalisePages(rawPages: KirbyPage[]): NormalisedPage[] {
...
const connector = {
  ...
  async fetch(_dataType: string, credentials: Creds, _params?: Record<string, unknown>): Promise<unknown> {
    ...
    let rawPages: KirbyPage[] | null = null;
  ...
  extractMetrics(data: unknown, _runAt?: string): Array<{ name: string; value: number; data: unknown }> {
    ...
    const pages = Array.isArray(d?.pages) ? d!.pages as NormalisedPage[] : [];
  ...
  async getAuthUrl(_state?: string): Promise<string> { throw new Error('Kirby uses Basic auth, not OAuth.'); },
  async exchangeCode(_code?: string): Promise<never> { throw new Error('Kirby uses Basic auth, not OAuth.'); },
  async refreshToken(_credentials?: Creds): Promise<never> { throw new Error('Kirby uses Basic auth, not OAuth.'); },
};
