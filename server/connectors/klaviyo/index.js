...
function klaviyoFetch(endpoint, apiKey, init = {}) {
...
const connector = {
  ...
  async healthCheck(credentials) {
    try {
      ...
      return {
        ok: true,
        details: {
          account: account?.attributes?.contact_information?.organization_name ?? null,
  ...
  async fetch(_dataType, credentials, _params) {
    ...
    try {
      ...
      placedOrderMetricId = placed?.id ?? null;
    } catch (err) {
      console.warn('[klaviyo] failed to resolve Placed Order metric id:', err.message);
    }
    ...
    try {
      ...
      console.warn('[klaviyo] campaigns fetch failed:', err.message);
    }
    ...
    if (placedOrderMetricId) {
      try {
        ...
        console.warn('[klaviyo] campaign values fetch failed:', err.message);
      }
    }
    ...
    try {
      ...
      console.warn('[klaviyo] flows fetch failed:', err.message);
    }
    ...
    if (placedOrderMetricId) {
      try {
        ...
        console.warn('[klaviyo] flow values fetch failed:', err.message);
      }
    }
    ...
    try {
      ...
      console.warn('[klaviyo] lists fetch failed:', err.message);
    }
  ...
  extractMetrics(data, _runAt) {
    ...
    const campaignValues = Array.isArray(data?.campaignValues) ? data.campaignValues : [];
    const flows = Array.isArray(data?.flows) ? data.flows : [];
    const flowValues = Array.isArray(data?.flowValues) ? data.flowValues : [];
    ...
    const sentRecent = campaigns.filter((c) => {
      const s = c.attributes?.send_time;
    ...
    const cvByCampaign = new Map();
    for (const row of campaignValues) {
      const id = row?.groupings?.campaign_id ?? row?.groupings?.['campaign_id'];
      if (id) cvByCampaign.set(id, row?.statistics ?? {});
    }
    ...
    for (const c of sentRecent) {
      const stats = cvByCampaign.get(c.id) ?? {};
    ...
    // Flows
    const fvByFlow = new Map();
    for (const row of flowValues) {
      const id = row?.groupings?.flow_id;
      if (id) fvByFlow.set(id, row?.statistics ?? {});
    }

    let flowRevenueTotal = 0;
    const revenueByClass = { abandoned_cart: 0, welcome: 0, post_purchase: 0, win_back: 0, browse_abandon: 0 };
    const flowRows = [];
    for (const f of flows) {
      const stats = fvByFlow.get(f.id) ?? {};
      const revenue = Number(stats.conversion_value ?? 0);
      const recipients = Number(stats.recipients ?? 0);
      flowRevenueTotal += revenue;
      const cls = classifyFlow(f.attributes?.name);
    ...
    for (const l of lists) {
      ...
      const count = Number(attrs.profile_count ?? 0);
      totalSubscribers += count;
      listRows.push({
        id: l.id,
        name: attrs.name ?? '(unnamed)',
        profile_count: count,
        opt_in_process: attrs.opt_in_process ?? null,
  ...
  async getAuthUrl() { throw new Error('Klaviyo uses API key authentication, not OAuth.'); },
  async exchangeCode() { throw new Error('Klaviyo uses API key authentication, not OAuth.'); },
  async refreshToken() { throw new Error('Klaviyo uses API key authentication, not OAuth.'); },
};
