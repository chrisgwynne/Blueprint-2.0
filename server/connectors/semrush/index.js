...
async function semrushFetch(params, apiKey, label) {
...
const connector = {
  ...
  async fetch(_dataType, credentials, _params) {
    ...
    const results = await Promise.allSettled([
      semrushFetch({
        type: 'domain_rank', domain, database,
  ...
  async getAuthUrl() { throw new Error('SEMrush uses API key authentication, not OAuth.'); },
  async exchangeCode() { throw new Error('SEMrush uses API key authentication, not OAuth.'); },
  async refreshToken() { throw new Error('SEMrush uses API key authentication, not OAuth.'); },
};
