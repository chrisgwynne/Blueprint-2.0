 ...
 * ServerConnection interface. All file operations must go through the
 * security.js gate before calling into here. This module is purposefully
 * narrow: connect, list, read, write, exec, disconnect. Nothing else.
 */
...
function streamToBuffer(readable, maxBytes = MAX_FILE_SIZE) {
...
class SshConnection {
  ...
  async connect() {
    ...
    await new Promise((resolveFn, rejectFn) => {
      ...
      this.client.once('error', (err) => { clearTimeout(to); rejectFn(err); });
      this.client.connect(connectOpts);
    });

    this.sftp = await new Promise((resolveFn, rejectFn) => {
      this.client.sftp((err, sftp) => err ? rejectFn(err) : resolveFn(sftp));
    });
  }
  ...
  async stat(remotePath) {
    ...
    return new Promise((resolveFn, rejectFn) => {
      this.sftp.stat(remotePath, (err, attrs) => err ? rejectFn(err) : resolveFn(attrs));
    });
  }
  ...
  async readFile(remotePath) {
    ...
    const buf = await new Promise((resolveFn, rejectFn) => {
...
class FtpConnection {
  ...
  async stat(remotePath) {
    ...
    return { size: f.size, mtime: f.modifiedAt?.getTime() / 1000 };
  }
 ...
 * Callers are responsible for calling .disconnect() in a finally block.
 */
export async function createServerConnection(credentials, config = {}) {
