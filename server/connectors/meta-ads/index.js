  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (!res.ok) {
        ...
        const err = new Error(`Meta API ${res.status}: ${text.substring(0, 300)}`);
 ...
 */
function getAction(arr, actionType) {
  if (!Array.isArray(arr)) return 0;
  return num(arr.find(a => a.action_type === actionType)?.value);
}
...
async function fetchAccount(credentials, config) {
  const creds = await ensureFreshToken(credentials);
  const accountId = config.adAccountId || credentials.ad_account_id;
...
async function fetchInsights(credentials, config) {
  const creds = await ensureFreshToken(credentials);
  const accountId = config.adAccountId || credentials.ad_account_id;
...
async function fetchCampaigns(credentials, config) {
  const creds = await ensureFreshToken(credentials);
  const accountId = config.adAccountId || credentials.ad_account_id;
  if (!accountId) throw new Error('Meta Ads: adAccountId is required.');
...
async function fetchAdSets(credentials, config) {
  const creds = await ensureFreshToken(credentials);
  const accountId = config.adAccountId || credentials.ad_account_id;
  if (!accountId) throw new Error('Meta Ads: adAccountId is required.');
...
const connector = {
  ...
  extractMetrics(data, _runAt) {
    ...
    const current = data.account?.data?.[0] ?? {};
    const prev = data.previous?.data?.[0] ?? {};
