...
function buildClusterPrompt(signals: SignalRow[], business: BusinessRow): string {
...
function loadConductorLLM(): Record<string, unknown> | null {
  const path = join(AGENTS_DIR, 'conductor', 'profile.yaml');
  if (!existsSync(path)) return null;
  try {
    const profile = yaml.load(readFileSync(path, 'utf8')) as Record<string, unknown> | null;
    return (profile?.llm as Record<string, unknown>) ?? null;
  } catch { return null; }
}
 ...
 * @returns IDs of created clusters.
 */
export async function runClustering(businessId: string): Promise<string[]> {
  ...
  const business = db.prepare('SELECT * FROM businesses WHERE id = ?').get(businessId) as BusinessRow | null;
  if (!business) return [];
  ...
  try {
    ...
    console.warn('[cluster-engine] LLM call failed:', (err as Error).message);
    return [];
  }
  ...
  for (const cluster of parsed.clusters) {
    ...
    const validIds = cluster.signal_ids.filter((id: string) => validSignalIds.has(id));
    ...
    try {
      ...
      console.warn('[cluster-engine] insert failed:', (err as Error).message);
      continue;
    }
    ...
    try {
      ...
      if (kbResult?.engine) {
        ...
        const signalList = validIds.map((id: string) => {
...
`;
      ...
      console.warn('[cluster-engine] KB write failed (non-fatal):', (err as Error).message);
    }
  }
