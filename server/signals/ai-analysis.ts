...
interface AIInsightProposedTask {
  title?: string;
  description?: string;
  action_type?: string;
...
interface AIInsight {
  ...
  description?: string;
  evidence?: string[];
  connectors_involved?: string[];
  confidence?: number;
  actionable?: boolean;
  priority?: string;
  proposed_task?: AIInsightProposedTask;
}
 ...
 */
function extractJSON(rawContent: string): ParsedLLMResponse | null {
  if (!rawContent) return null;
  const text = String(rawContent).trim();

  // Strategy 1: direct parse
  try { return JSON.parse(text) as ParsedLLMResponse; } catch {}
  ...
  if (firstBrace !== -1 && lastBrace > firstBrace) {
    try { return JSON.parse(text.slice(firstBrace, lastBrace + 1)) as ParsedLLMResponse; } catch {}
  }
...
function resolveAnalysisProvider(): { provider: string; model: string } {
  const stored = db.prepare("SELECT value FROM settings WHERE key = 'ai_analysis_provider'").get() as { value: string } | null;
  if (stored?.value) {
    try {
      const { provider, model } = JSON.parse(stored.value) as { provider?: string; model?: string };
  ...
  const resolved = resolveProfileLLM(null);
...
function v(latestByName: LatestByName, key: string, fallback: unknown = 'N/A'): unknown {
  const m = latestByName[key];
  if (!m) return fallback;
  return m.value !== null && m.value !== undefined ? m.value : fallback;
}

function vData(latestByName: LatestByName, key: string): unknown {
  return latestByName[key]?.data ?? null;
}

function buildCustomConnectorSummary(type: string, latestByName: LatestByName): string | null {
  if (type === 'uptimerobot') {
    const downData = (vData(latestByName, 'uptimerobot.down_monitors_data') ?? []) as Array<{ friendly_name?: string }>;
...
Down monitors: ${downNames || 'none'}`;
  ...
  if (type === 'todoist') {
    const overdueData = (vData(latestByName, 'todoist.overdue_tasks') ?? []) as Array<{ priority?: number }>;
...
Avg delivery rate: ${(parseFloat(String(v(latestByName, 'stannp.avg_delivery_rate', 0))) * 100).toFixed(1)}%
...
Impression share: ${(parseFloat(String(v(latestByName, 'google-ads.impression_share', 0))) * 100).toFixed(1)}%`;
  }

  if (type === 'meta-ads') {
    const metaCampaigns = latestByName['meta-ads.campaigns_data']?.metric_data as string | unknown[] | undefined;
    let campaignCount = 0;
    try {
    ...
    const ctr = parseFloat(String(v(latestByName, 'meta-ads.ctr', 0)));
...
Avg CTR: ${(parseFloat(String(v(latestByName, 'gsc.avg_ctr', 0))) * 100).toFixed(2)}%
...
Bounce rate: ${(parseFloat(String(v(latestByName, 'ga4.bounce_rate', 0))) * 100).toFixed(1)}%
Conversions: ${v(latestByName, 'ga4.conversions')} (prev: ${v(latestByName, 'ga4.conversions_prev')})`;
  }

  if (type === 'klaviyo') {
    const openRate = parseFloat(String(v(latestByName, 'klaviyo.campaign_open_rate', 0)));
    const clickRate = parseFloat(String(v(latestByName, 'klaviyo.campaign_click_rate', 0)));
    const unsubRate = parseFloat(String(v(latestByName, 'klaviyo.unsubscribe_rate_30d', 0)));
...
Live flows: ${v(latestByName, 'klaviyo.flows_live_count')}`;
  ...
  if (type === 'semrush') {
    const opsData = (vData(latestByName, 'semrush.opportunities_data') ?? []) as Array<{ keyword?: string; position?: number; search_volume?: number }>;
...
Top page-2 opportunities: ${topOps || 'none'}`;
  ...
  if (type === 'social') {
    const fbEng = parseFloat(String(v(latestByName, 'fb.page_engagement_rate', 0)));
    const igEng = parseFloat(String(v(latestByName, 'ig.avg_post_engagement_rate', 0)));
...
Instagram profile views (30d): ${v(latestByName, 'ig.profile_views_30d')} → website clicks: ${v(latestByName, 'ig.website_clicks_30d')}`;
  ...
  if (type === 'buffer') {
    const topPlatMeta = vData(latestByName, 'buffer.top_performing_platform') as { platform?: string; avg_engagement?: unknown } | null;
    const topPlatLabel = topPlatMeta?.platform ? `${topPlatMeta.platform} (avg ${topPlatMeta.avg_engagement})` : 'none';
    const nextMeta = vData(latestByName, 'buffer.next_scheduled_post') as { iso?: string } | null;
...
Sessions (30d): ${v(latestByName, 'wix.sessions_30d')}, pageviews: ${v(latestByName, 'wix.pageviews_30d')}`;
  ...
  if (type === 'server-access') {
    const errors = (vData(latestByName, 'server.recent_errors') ?? []) as Array<{ type?: string; line?: string }>;
    const fatalNames = Array.isArray(errors)
      ? errors.filter((e) => e.type === 'fatal').slice(0, 3).map((e) => (e.line || '').slice(0, 120)).join(' | ')
      : '';
    const external = (vData(latestByName, 'server.external_changes') ?? []) as unknown[];
...
function buildShopifySummary(latestByName: LatestByName): string {
  const revenue   = latestByName['revenue']?.value as number | undefined;
  const orders    = latestByName['orders']?.value;
  const aov       = latestByName['aov']?.value as number | undefined;
  const customers = latestByName['customers']?.value;

  const dailyRevenue  = (latestByName['daily_revenue']?.data  ?? []) as DailyRevenueEntry[];
  const dailyOrders   = (latestByName['orders_daily']?.data   ?? []) as DailyOrderEntry[];
  const topProducts   = (latestByName['top_products_data']?.data ?? []) as TopProduct[];

  // Period summaries
  const sumRevenue = (arr: DailyRevenueEntry[], from: number, to?: number) =>
    arr.slice(from, to).reduce((s, d) => s + (d.amount || 0), 0);
  const sumOrders = (arr: DailyOrderEntry[], from: number, to?: number) =>
    arr.slice(from, to).reduce((s, d) => s + (d.count  || 0), 0);
...
export async function runAIAnalysis(
  businessId: string,
  runId: string
): Promise<{ runId: string; insightsCount: number; tasksCreated: number; healthScore: number | null; summary: string }> {
  try {
    // 1. Gather data
    const business = db.prepare('SELECT * FROM businesses WHERE id = ?').get(businessId) as { name: string; type?: string; description?: string } | null;
    ...
    ).all(businessId) as Array<{ id: string; type: string; name: string; status: string; last_sync: string | null }>;
    ...
    const metricsRaw = connectorIds.length > 0
      ? db.prepare(`
        ...
        `).all(businessId, since28d) as Array<{ connector_id: string; metric_name: string; metric_value: unknown; metric_data: string | null; recorded_at: string }>
      : [];

    // Group metrics by connector type
    const metricsByConnector: Record<string, MetricEntry[]> = {};
    for (const m of metricsRaw) {
      ...
      metricsByConnector[key].push({
        ...
        data: m.metric_data ? JSON.parse(m.metric_data) as unknown : null,
    ...
    `).all(businessId) as Array<{ rule_id: string; type: string; severity: string; title: string; description: string | null; confidence: number | null; created_at: string }>;
...
${Object.keys(metricsByConnector).length > 0
  ? Object.entries(metricsByConnector).map(([type, metrics]) => {
      const latestByName: LatestByName = {};
...
Identify insights and proposed tasks based solely on this data.`;
    ...
    const insights: AIInsight[] = Array.isArray(parsed.insights) ? parsed.insights : [];
      ...
      for (const insight of validInsights) {
        ...
        if (
          ...
          try {
            ...
            console.error('[ai-analysis] Failed to create task for insight:', (taskErr as Error).message);
          }
        }
      }
    });
    ...
    try {
      const conductorRow = db.prepare("SELECT id FROM agents WHERE id = 'conductor'").get() as { id: string } | null;
      if (conductorRow) {
      ...
      console.warn('[ai-analysis] Failed to log agent_run for conductor:', (logErr as Error).message);
    }
 ...
 */
export async function getAnalysisStatus(runId: string): Promise<Record<string, unknown> | null> {
  return (db.prepare('SELECT * FROM analysis_runs WHERE id = ?').get(runId) as Record<string, unknown> | null) ?? null;
}
