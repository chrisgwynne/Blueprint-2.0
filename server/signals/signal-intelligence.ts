...
interface ConnectorImplication {
  connector: string;
  reason: string;
}
...
const SIGNAL_CONNECTOR_IMPLICATIONS: Record<string, ConnectorImplication> = {
...
export async function processNewSignal(
  signalId: string,
  businessId: string,
  opts: { skipAgentTrigger?: boolean } = {}
): Promise<void> {
  if (!signalId || !businessId) return;

  const signal = db.prepare('SELECT * FROM signals WHERE id = ?').get(signalId) as SignalRow | null;
  if (!signal) return;
...
async function fileSignalToKB(signal: SignalRow, businessId: string): Promise<void> {
  try {
    type KBWriteEngine = {
      writeFile(path: string, content: string, frontmatter?: Record<string, unknown> | null, commitMessage?: string | null): Promise<void>;
    };
    const { getKBForBusiness } = await import('../kb/kb-config.js') as {
      getKBForBusiness: (id: string) => Promise<{ engine: KBWriteEngine } | null>;
    };
...
`;
    ...
    console.warn('[signal-intel] fileSignalToKB failed:', (err as Error).message);
  }
}

// ─── 2. Goal impact ──────────────────────────────────────────────────────────

async function checkGoalImpact(signal: SignalRow, businessId: string): Promise<void> {
  try {
    ...
    if (signal.connector_id) {
      ...
      ).get(signal.connector_id) as { type: string } | null;
    ...
    const goals = likePattern
      ? db.prepare(`
        ...
        `).all(businessId, likePattern) as Array<{ id: string; title: string; metric_name: string; progress_pct: number | null }>
      : [];
    ...
    for (const goal of goals) {
      ...
      if (signal.severity === 'critical') {
        const { createTask } = await import('../tasks/task-queue.js') as unknown as {
          createTask: (params: Record<string, unknown>) => { id: string } | null;
        };
        ...
        const existing = db.prepare(`
          SELECT id FROM tasks
           WHERE business_id = ?
             ...
             AND title = ?
        `).get(businessId, title) as { id: string } | null;
        ...
        try {
          ...
          console.warn('[signal-intel] goal-at-risk task failed:', (err as Error).message);
        }
      }
    }
  } catch (err) {
    console.warn('[signal-intel] checkGoalImpact failed:', (err as Error).message);
  }
}

// ─── 3. Trigger relevant agents ──────────────────────────────────────────────

async function triggerRelevantAgents(signal: SignalRow, businessId: string): Promise<void> {
  ...
  try {
    ...
    for (const agent of agents) {
      ...
      const triggers: string[] = profile?.signal_triggers ?? [];
      ...
      const recent = db.prepare(`
        SELECT id FROM agent_runs
         ...
         LIMIT 1
      `).get(agent.id, businessId, AGENT_COOLDOWN_MINUTES) as { id: string } | null;
      if (recent) {
        logIntelligenceEvent({
          business_id: businessId,
          source_type: 'signal',
      ...
      (async () => {
        try {
          const { runAgent } = await import('../agents/agent-runner.js') as {
            runAgent: (agentId: string, businessId: string, triggerType?: string, triggerId?: string) => Promise<unknown>;
          };
          ...
          console.warn(`[signal-intel] runAgent ${agent.id} failed:`, (err as Error).message);
        }
      })();
    }
  } catch (err) {
    console.warn('[signal-intel] triggerRelevantAgents failed:', (err as Error).message);
  }
}
 ...
 */
function loadAgentProfiles(agentIds: string[]): Map<string, AgentProfile> {
  ...
  for (const id of agentIds) {
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object') out.set(id, parsed as AgentProfile);
    } catch {}
  }
  return out;
}
...
async function checkConnectorImplications(signal: SignalRow, businessId: string): Promise<void> {
  ...
  try {
    ...
    console.warn('[signal-intel] checkConnectorImplications failed:', (err as Error).message);
  }
}
