...
const COOLDOWN_OVERRIDE_EVENTS = new Set([
  'signal.critical',
  'signal.alert',
...
const CONDUCTOR_GENERAL_EVENTS = new Set([
  'signal.critical',
  'signal.alert',
  'signal.created',
  'task.approved',
 ...
 *
 * @param {string} eventType    e.g. 'connector.sync.complete', 'signal.critical',
 *                              'task.approved', 'goal.at_risk', 'agent.brief.immediate'
 * @param {object} eventData    event-specific payload:
 *                                - connector.sync.*: { connector_type, connector_id? }
 *                                - signal.*: { signal_id, connector_type? }
 *                                - task.*: { task_id, action_type? }
 *                                - goal.*: { goal_id }
 *                                - agent.brief.immediate: { to_agent }
 *                                - chat.mention: { agent_id, conversation_id? }
 * @param {string} businessId
 */
export async function dispatchAgentEvent(eventType, eventData = {}, businessId) {
  ...
  try {
    ...
    for (const target of targets) {
      ...
      wakeAgent({
        ...
        console.warn(`[event-triggers] wake ${target.agentId} for ${eventType} failed:`, err.message);
      });
    }
  } catch (err) {
    console.warn(`[event-triggers] dispatchAgentEvent(${eventType}) failed:`, err.message);
  }
}
 ...
 */
export function resolveEventTargets(eventType, eventData = {}) {
  const targets = [];
  const seen = new Set();
  const push = (agentId, trigger, triggerId = null) => {
    if (!agentId) return;
...
async function wakeAgent({ agentId, businessId, trigger, triggerId, overrideCooldown }) {
  ...
  if (!overrideCooldown) {
    ...
    const recent = db.prepare(`
      SELECT id FROM agent_runs
       ...
       LIMIT 1
    `).get(agentId, businessId, cooldownMin);
  ...
  runAgent(agentId, businessId, trigger, triggerId).catch((err) => {
    console.warn(`[event-triggers] runAgent(${agentId}) failed:`, err.message);
  });
}
