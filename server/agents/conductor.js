 ...
 *
 * @param {Object[]} agents - DB agent rows
 * @param {Object[]} signals - Open signals
 * @param {Map<string, Object>} profileMap - agentId → parsed YAML profile
 * @returns {Array<{ agent, signal }>}
 */
function matchSignalsToAgents(agents, signals, profileMap) {
  const matches = [];
  const seen = new Set(); // Prevent same agent running twice for same signal type
 ...
 */
function getDueScheduledJobs(agent, profile) {
 ...
 * @returns {Promise<{ runs: Array, errors: Array }>}
 */
export async function runConductor(businessId) {
  ...
  for (const { agent, signal } of signalMatches) {
    try {
      ...
      console.error(`[conductor] Agent '${agent.id}' failed for signal '${signal.id}':`, err.message);
      errors.push({ agentId: agent.id, trigger: 'signal', signalId: signal.id, error: err.message });
    }
  }
  ...
  for (const agent of agents) {
    ...
    for (const job of dueJobs) {
      ...
      try {
        ...
        errors.push({ agentId: agent.id, trigger: 'schedule', jobId: job.id, error: err.message });
      }
    }
  }
 ...
 */
export async function runConductorAllBusinesses() {
  const businesses = db.prepare('SELECT id, name FROM businesses').all();
  const allResults = [];
  ...
  for (const business of businesses) {
    try {
      ...
      console.error(`[conductor] Failed for business '${business.name}':`, err.message);
      allResults.push({ businessId: business.id, businessName: business.name, error: err.message });
    }
  }
