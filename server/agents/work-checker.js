 ...
 * @returns {{hasWork: boolean, reasons: string[], lastRunAt: string|null}}
 */
export function hasWorkToDo(agentId, businessId) {
 ...
 */
export function categoriseTrigger(trigger) {
...
function getLastSuccessfulRunAt(agentId, businessId) {
  try {
...
function countActionableInbox(agentId, minPriority, businessId) {
  const path = join(AGENTS_DIR, agentId, 'inbox.jsonl');
  if (!existsSync(path)) return 0;
  const RANK = { fyi: 0, next_run: 1, immediate: 2 };
...
function connectorSyncsSince(types, businessId, lastRunAt) {
    ...
    if (lastRunAt) {
      return db.prepare(`
        SELECT c.type, cs.created_at
         ...
         LIMIT 5
      `).all(businessId, ...types, lastRunAt);
    }
    return db.prepare(`
      SELECT c.type, cs.created_at
        FROM connector_syncs cs
       ...
       ORDER BY cs.created_at DESC
       LIMIT 5
    `).all(businessId, ...types);
  } catch {
    return [];
  }
}

function signalsSince(connectorTypes, severities, businessId, lastRunAt) {
    ...
    if (lastRunAt) {
      return db.prepare(`
        SELECT s.id, s.title, s.severity
         ...
         LIMIT 5
      `).all(businessId, ...connectorTypes, ...severities, lastRunAt);
    }
    return db.prepare(`
      SELECT s.id, s.title, s.severity
        FROM signals s
       ...
       ORDER BY s.created_at DESC
       LIMIT 5
    `).all(businessId, ...connectorTypes, ...severities);
  } catch {
    return [];
  }
}

function mentionsSince(agentId, businessId, lastRunAt) {
  try {
      ...
      return db.prepare(`
        SELECT COUNT(*) as n FROM chat_messages
         WHERE business_id = ?
           ...
           AND content LIKE ?
           AND created_at > ?
      `).get(businessId, pattern, lastRunAt)?.n ?? 0;
    }
    return db.prepare(`
      SELECT COUNT(*) as n FROM chat_messages
       WHERE business_id = ?
    ...
    `).get(businessId, pattern)?.n ?? 0;
  } catch {
    return 0;
  }
}

function assignedTasksPending(agentId, businessId) {
  try {
    return db.prepare(`
 ...
 */
function addConductorReasons(reasons, businessId, lastRunAt) {
  try {
    ...
    console.warn('[work-checker] conductor checks failed:', err.message);
  }
}
