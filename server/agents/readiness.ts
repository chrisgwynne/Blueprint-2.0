...
interface ReadinessResult {
  ...
  last_sync_ages: Record<string, number | null>;
  reason: string;
}
...
function loadProfile(agentId: string): AgentProfile | null {
  const profilePath = join(AGENTS_DIR, agentId, 'profile.yaml');
  if (!existsSync(profilePath)) return null;
  try { return yaml.load(readFileSync(profilePath, 'utf8')) as AgentProfile; }
  catch { return null; }
}
...
function getConnectorStatus(
  ...
  const lastSync = db.prepare(
    `SELECT created_at FROM connector_syncs
     ...
     ORDER BY created_at DESC LIMIT 1`
  ).get(connector.id) as SyncRow | null;
...
function hoursSince(isoLike: string | null | undefined): number | null {
...
function readAgentRowStatus(agentId: string): string | null {
  const row = db.prepare('SELECT status FROM agents WHERE id = ?').get(agentId) as AgentStatusRow | null;
  return row?.status ?? null;
}

export function checkAgentReadiness(agentId: string, businessId: string): ReadinessResult {
  ...
  const lastSyncAges: Record<string, number | null> = {};
 ...
 */
export function findPromotableAgents(businessId: string): Array<{ id: string; readiness: ReadinessResult }> {
  const pending = db.prepare(
    `SELECT id FROM agents WHERE status = 'pending'`
  ).all() as AgentIdRow[];
  const promotable: Array<{ id: string; readiness: ReadinessResult }> = [];
