...
export function getEmailProvider(): string {
  const row = db.prepare("SELECT value FROM settings WHERE key = 'email_provider'").get() as { value: string } | undefined;
  if (!row) return 'smtp';
...
export function getEmailRecipient(): string | null {
  const row = db.prepare("SELECT value FROM settings WHERE key = 'email_report_to'").get() as { value: string } | undefined;
  if (!row) return null;
  try { return JSON.parse(row.value); } catch { return null; }
}

export function saveEmailRecipient(email: string): void {
  db.prepare(`
...
export function saveEmailProvider(provider: string): void {
  db.prepare(`
 ...
 */
export async function sendEmail({ to, subject, html, text, from }: EmailMessage): Promise<EmailResult> {
...
async function sendViaSMTP({ from, to, subject, html, text, config }: { from: string; to: string; subject: string; html?: string; text: string; config: SmtpConfig }): Promise<EmailResult> {
...
async function sendViaResend({ from, to, subject, html, text, config }: { from: string; to: string; subject: string; html?: string; text: string; config: ApiKeyConfig }): Promise<EmailResult> {
...
async function sendViaPostmark({ from, to, subject, html, text, config }: { from: string; to: string; subject: string; html?: string; text: string; config: ApiKeyConfig }): Promise<EmailResult> {
...
async function sendViaBrevo({ from, to, subject, html, text, config }: { from: string; to: string; subject: string; html?: string; text: string; config: ApiKeyConfig }): Promise<EmailResult> {
