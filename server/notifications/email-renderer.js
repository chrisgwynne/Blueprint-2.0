 ...
 */
export function renderWeeklyBriefingEmail(briefingData) {
  const {
    business = {},
    period = '',
    health_score = 0,
        ...
        <div>
          ...
          <div class="task-meta">
            ...
            ${t.verdict === 'improved' && t.change_pct != null
              ? `${safe(t.metric_label)} ${t.change_pct > 0 ? '+' : ''}${Math.round(t.change_pct)}%`
 ...
 */
export function renderClientBriefingEmail(briefingData) {
  const {
    business = {},
    period = '',
    health_score = 0,
