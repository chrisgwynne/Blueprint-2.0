...
const PHASE4_MIGRATIONS: string[] = [
  ...
  `ALTER TABLE tasks ADD COLUMN canonical_task_key TEXT`,
  `CREATE INDEX IF NOT EXISTS idx_tasks_canonical_key ON tasks(business_id, canonical_task_key)`,
  ...
  `ALTER TABLE signals ADD COLUMN canonical_fingerprint TEXT`,
  `ALTER TABLE signals ADD COLUMN condition_fingerprint TEXT`,
  `ALTER TABLE signals ADD COLUMN canonical_generation INTEGER NOT NULL DEFAULT 1`,
  `CREATE INDEX IF NOT EXISTS idx_signals_canonical_active ON signals(business_id, canonical_fingerprint, status)`,
  `CREATE TABLE IF NOT EXISTS signal_suppressions (id TEXT PRIMARY KEY, business_id TEXT NOT NULL REFERENCES businesses(id), canonical_fingerprint TEXT NOT NULL, condition_fingerprint TEXT NOT NULL, source_signal_id TEXT REFERENCES signals(id), reason TEXT NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`,
  `CREATE INDEX IF NOT EXISTS idx_signal_suppressions_lookup ON signal_suppressions(business_id, canonical_fingerprint, condition_fingerprint)`,
