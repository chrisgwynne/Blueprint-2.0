...
function setDownloadHeaders(res: Response, filename: string, contentType = 'text/csv'): void {
...
router.get('/:businessId/metrics', (req: Request, res: Response) => {
  try {
    const businessId = req.params.businessId as string;
    const { connector } = req.query;
    const filename = `metrics-${businessId.slice(0, 8)}-${new Date().toISOString().split('T')[0]}.csv`;
...
router.get('/:businessId/tasks', (req: Request, res: Response) => {
  try {
    const businessId = req.params.businessId as string;
    const filename = `tasks-${businessId.slice(0, 8)}-${new Date().toISOString().split('T')[0]}.csv`;
    setDownloadHeaders(res, filename);
...
router.get('/:businessId/signals', (req: Request, res: Response) => {
  try {
    const businessId = req.params.businessId as string;
    const filename = `signals-${businessId.slice(0, 8)}-${new Date().toISOString().split('T')[0]}.csv`;
    setDownloadHeaders(res, filename);
...
router.get('/:businessId/agent-runs', (req: Request, res: Response) => {
  try {
    const businessId = req.params.businessId as string;
    const filename = `agent-runs-${businessId.slice(0, 8)}-${new Date().toISOString().split('T')[0]}.csv`;
    setDownloadHeaders(res, filename);
...
router.get('/database', (req: Request, res: Response) => {
  try {
    ...
    const tables = (db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'").all() as any[]).map(r => r.name as string);
    ...
    for (const table of tables) {
      ...
      const rows = db.prepare(`SELECT * FROM ${table} LIMIT 10000`).all() as any[];
