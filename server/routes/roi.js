...
router.get('/:businessId/agents', (req, res) => {
  try {
    const { businessId } = req.params;
    const windowDays = Math.max(7, Math.min(365, parseInt(req.query.window_days, 10) || 90));
...
router.get('/:businessId/baselines', (req, res) => {
  try {
    const { businessId } = req.params;
...
// baseline_date.
router.get('/:businessId/trajectory', (req, res) => {
  try {
    const { businessId } = req.params;
...
router.get('/:businessId/assessment', async (req, res) => {
  try {
    const { businessId } = req.params;
    const report = computeROIReport(businessId);
    if (!report) return res.status(404).json({ error: 'Business not found' });
    ...
    try {
      const { resolveProfileLLM, runLLM } = await import('../lib/llm-providers.js');
      ...
      console.warn('[roi/assessment] LLM unavailable, serving template:', err.message);
    }
...
function buildAssessmentPrompt(report, agentRows) {
...
${unattributed.slice(0, 3).map(r => `- ${r.metric_name} ${r.change_pct?.toFixed?.(1)}% — no Blueprint cause identified`).join('\n') || '- none'}
...
function buildTemplateAssessment(report, agentRows) {
  const parts = [];
