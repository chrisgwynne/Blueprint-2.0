 ...
 * sparkline series.
 */
function recentMetric(businessId, metricName, limit = 2) {
  return db.prepare(`
...
function sparkline(businessId, metricName, days = 14) {
  return db.prepare(`
...
function latestData(businessId, metricName) {
  ...
  try { return JSON.parse(row.metric_data); } catch { return null; }
}
...
router.get('/:businessId', (req, res) => {
  try {
    ...
    const top_keywords = (latestData(businessId, 'gsc.keywords') ?? []).slice(0, 10).map(k => ({
    ...
    let top_pages = (latestData(businessId, 'ga4.top_pages') ?? []).slice(0, 10).map(p => ({
    ...
    `).all(businessId).map(s => ({ ...s, data: parseJsonField(s.data) }));
