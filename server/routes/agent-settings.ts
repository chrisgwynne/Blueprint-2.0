 ...
 */
router.get('/poll-intervals', (req: Request, res: Response) => {
  try {
    const agents = db.prepare("SELECT id, name FROM agents ORDER BY name").all() as Array<{ id: string; name: string }>;
    const out = agents.map((a) => {
      const override = db.prepare(
        "SELECT value FROM settings WHERE key = ?"
      ).get(`agent_poll_interval_${a.id}`) as { value: string } | undefined;
      const override_minutes = override?.value ? (() => {
        try { return Number(JSON.parse(override.value)); } catch { return null; }
      })() : null;
      const default_minutes = (DEFAULT_POLL_INTERVALS_MINUTES as Record<string, number>)[a.id] ?? 60;
 ...
 */
router.put('/poll-intervals/:agentId', (req: Request, res: Response) => {
  try {
 ...
 */
router.delete('/poll-intervals/:agentId', (req: Request, res: Response) => {
  try {
    const agentId = String(req.params.agentId);
