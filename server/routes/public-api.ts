...
function keyPrefix(key: unknown): string { return String(key).slice(0, 12); }

async function apiKeyAuth(req: Request, res: Response, next: NextFunction): Promise<void> {
  const header = req.headers.authorization;
  if (!header?.startsWith('Bearer bpk_')) {
    res.status(401).json({ error: 'Authorization: Bearer bpk_xxx header required.' });
    return;
  }
...
function rateLimit(req: Request, res: Response, next: NextFunction): void {
  ...
  const limit: number = req.apiKey?.rate_limit ?? 1000;
  ...
  if (calls.length > limit) {
    res.status(429).json({ error: 'Rate limit exceeded.', retry_after_seconds: 3600 });
    return;
  }
  next();
}
...
function safeJSON(val: unknown, fallback: any = []): any {
...
router.get('/businesses/:id/signals', (req: Request, res: Response) => {
  ...
  const rows = db.prepare(`SELECT * FROM signals WHERE ${conditions.join(' AND ')} ORDER BY created_at DESC LIMIT ?`).all(...params, lim) as any[];
...
router.post('/businesses/:id/signals', requireScope('write'), (req: Request, res: Response) => {
  ...
  try {
    import('../bap/webhook-dispatcher.js').then((m: any) =>
      m.dispatchWebhookEvent('signal.created', { signal_id: id, business_id: String(req.params.id), type, severity, title, confidence })
    );
  } catch {}
...
router.patch('/signals/:id', requireScope('write'), (req: Request, res: Response) => {
...
router.get('/businesses/:id/tasks', (req: Request, res: Response) => {
  const { status, limit = 50 } = req.query;
  const conditions = ['business_id = ?'];
  const params: any[] = [String(req.params.id)];
  if (status) { conditions.push('status = ?'); params.push(status); }
  const rows = db.prepare(`SELECT * FROM tasks WHERE ${conditions.join(' AND ')} ORDER BY created_at DESC LIMIT ?`)
    .all(...params, Math.min(parseInt(String(limit), 10) || 50, 200)) as any[];
...
router.post('/businesses/:id/tasks', requireScope('write'), async (req: Request, res: Response) => {
  try {
    ...
    const task = createTask({
      business_id: String(req.params.id), title, description, proposed_by: `api:${req.apiKey!.id}`,
...
router.patch('/tasks/:id/approve', requireScope('admin'), async (req: Request, res: Response) => {
  try {
    const { approveTask } = await import('../tasks/task-queue.js') as unknown as { approveTask: (id: string, by: string) => any };
    const task = approveTask(String(req.params.id), `api:${req.apiKey!.id}`);
    res.json({ task_id: task.id, status: task.status });
  } catch (err) {
    res.status((err as Error).message.includes('not found') ? 404 : 422).json({ error: (err as Error).message });
  }
});

router.patch('/tasks/:id/reject', requireScope('admin'), async (req: Request, res: Response) => {
  try {
    const { rejectTask } = await import('../tasks/task-queue.js') as unknown as { rejectTask: (id: string, by: string, reason: string) => any };
    const task = rejectTask(String(req.params.id), `api:${req.apiKey!.id}`, req.body.reason ?? '');
    res.json({ task_id: task.id, status: task.status });
  } catch (err) {
    res.status((err as Error).message.includes('not found') ? 404 : 422).json({ error: (err as Error).message });
  }
 ...
 */
router.post('/businesses/:id/metrics', requireScope('write'), (req: Request, res: Response) => {
...
router.post('/businesses/:id/ingest/zapier', requireScope('write'), async (req: Request, res: Response) => {
...
router.get('/businesses/:id/kb/search', async (req: Request, res: Response) => {
  try {
    const { getKBForBusiness } = await import('../kb/kb-config.js') as unknown as { getKBForBusiness: (id: string) => Promise<any> };
    const result = await getKBForBusiness(String(req.params.id));
...
router.get('/businesses/:id/kb/file/*', async (req: Request, res: Response) => {
  try {
    const { getKBForBusiness } = await import('../kb/kb-config.js') as unknown as { getKBForBusiness: (id: string) => Promise<any> };
    const result = await getKBForBusiness(String(req.params.id));
    if (!result) return res.status(404).json({ error: 'KB not initialized.' });
    ...
    res.status((err as Error).message.includes('not found') ? 404 : 500).json({ error: (err as Error).message });
  }
});

router.post('/businesses/:id/kb/file/*', requireScope('write'), async (req: Request, res: Response) => {
  try {
    const { getKBForBusiness } = await import('../kb/kb-config.js') as unknown as { getKBForBusiness: (id: string) => Promise<any> };
    const result = await getKBForBusiness(String(req.params.id));
    if (!result) return res.status(404).json({ error: 'KB not initialized.' });
...
router.get('/businesses/:id/connectors', (req: Request, res: Response) => {
  const rows = db.prepare('SELECT id, type, name, status, last_sync, last_error, config FROM connectors WHERE business_id = ?').all(String(req.params.id)) as any[];
  res.json({ connectors: rows.map((r) => ({ ...r, config: safeJSON(r.config, {}) })) });
});

router.post('/businesses/:id/connectors/:connectorId/sync', requireScope('write'), async (req: Request, res: Response) => {
  try {
    // Fire-and-forget sync
    const { syncConnector } = await import('../jobs/scheduler.js') as unknown as { syncConnector: (connector: any) => Promise<void> };
    const connector = db.prepare('SELECT * FROM connectors WHERE id = ? AND business_id = ?').get(String(req.params.connectorId), String(req.params.id));
...
): Promise<{ id: string; api_key: string; prefix: string; name: string; scopes: string[]; rate_limit: number }> {
