...
router.get('/:businessId/agents', (req, res) => {
  try {
    ...
    const perf = agents.map((agent) => {
      ...
      `).get(businessId, agent.id, thirtyDaysAgo)?.n ?? 0;
      ...
      const outcomeRows = db.prepare(`
        ...
        AND o.created_at >= ?
      `).all(businessId, agent.id, thirtyDaysAgo);
      ...
      const avgConfidence = db.prepare(`
        ...
        AND created_at >= ?
      `).get(businessId, agent.id, thirtyDaysAgo)?.avg ?? 0;
...
router.get('/:businessId/timeline', (req, res) => {
  try {
    const { businessId } = req.params;
    const metric = req.query.metric || 'gsc.total_clicks';
