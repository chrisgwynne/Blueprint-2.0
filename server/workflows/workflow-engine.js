...
export async function startWorkflow(workflowId, businessId, triggeredBy, reason = null) {
  ...
  executeNextStep(runId, businessId).catch((err) => {
...
async function executeNextStep(runId, businessId) {
  ...
  try {
    ...
    const previousOutput = context.last_step_output || '';
...
async function runAgentForStep(agentId, taskInput, businessId, workflowId, runId, stepRunId) {
  ...
  const runRow = db.prepare('SELECT reasoning FROM agent_runs WHERE id=?').get(result.runId);
  ...
  `).all(businessId, agentId, result.runId).map((r) => r.id);
...
export async function approveWorkflowStep(runId, stepIndex, approvedBy) {
  const run = db.prepare('SELECT * FROM workflow_runs WHERE id=?').get(runId);
...
export async function rejectWorkflowStep(runId, stepIndex, reason, rejectedBy = 'human') {
  const run = db.prepare('SELECT * FROM workflow_runs WHERE id=?').get(runId);
  if (!run) throw new Error('Run not found');
...
async function fileWorkflowToKB(runId, workflow, businessId) {
  try {
...
async function sendApprovalNotification(runId, step, workflow) {
  try {
