...
interface Evidence {
  ...
  seasonal: { within_seasonal: boolean };
}

/**
 * Gather raw evidence for a signal's metric change.
 */
function gatherEvidence(signal: Record<string, unknown>, businessId: string): Evidence {
  const data = typeof signal.data === 'string' ? (() => { try { return JSON.parse(signal.data as string) as Record<string, unknown>; } catch { return {} as Record<string, unknown>; } })() : ((signal.data as Record<string, unknown>) || {});
  const metricName = (data.metric_name ?? data.metric ?? null) as string | null;
  const changePct = (data.change_pct ?? data.pct_change ?? null) as number | null;
...
function buildPrompt(signal: Record<string, unknown>, evidence: Evidence): string {
  const lines: string[] = [];
  lines.push(`Signal: "${signal.title}"`);
  lines.push(`Description: ${(signal.description as string | null) ?? '(none)'}`);
  ...
  for (const c of evidence.candidates) {
    const daysSince = Math.ceil((Date.now() - new Date(c.measurement_window_start as string).getTime()) / 86400000);
    const inWindow = daysSince >= (c.min_days as number) && daysSince <= (c.expected_days as number) * 1.5;
 ...
 */
export async function analyseAndStoreSignalAttribution(signalId: string): Promise<Record<string, unknown> | null> {
  const signal = db.prepare('SELECT * FROM signals WHERE id = ?').get(signalId) as Record<string, unknown> | null;
  if (!signal) return null;
  return analyseAndStore(signal);
}

export async function analyseAndStore(signal: Record<string, unknown>): Promise<Record<string, unknown> | null> {
  try {
    const evidence = gatherEvidence(signal, signal.business_id as string);
    ...
    const doNotAct = (parsed.do_not_act_until as string | null) || null;
    ...
    console.warn('[attribution] analyse failed:', (err as Error).message);
    return null;
  }
}
