 ...
 */
export function seedActionWindows() {
 ...
 */
export function recordActionMemory(task) {
  ...
  const windowStart = new Date(task.completed_at || Date.now());
  const windowEnd = new Date(windowStart);
  windowEnd.setDate(windowEnd.getDate() + window.expected_days);

  const doNotTouchUntil = new Date(windowStart);
  doNotTouchUntil.setDate(doNotTouchUntil.getDate() + window.min_days);
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    id, task.business_id, task.id,
    task.action_type, task.title, task.description ?? null,
    payload.url ?? null, payload.entity ?? null,
    window.metric_types,
 ...
 */
export function markMeasurementReady() {
