...
interface TemporalActionWithDays extends TemporalAction {
  days_until_measurable: number;
  days_until_window_close: number;
}

interface TemporalState {
  in_flight: TemporalAction[];
  ready_for_measurement: TemporalAction[];
  still_waiting: TemporalActionWithDays[];
}

export function getTemporalState(businessId: string): TemporalState {
  ...
  `).all(businessId) as TemporalAction[];
  ...
  return {
    ...
    still_waiting: stillWaiting.map((a) => ({
      ...a,
      days_until_measurable: Math.ceil((new Date(a.do_not_touch_until).getTime() - now) / 86400000),
      days_until_window_close: Math.ceil((new Date(a.measurement_window_end).getTime() - now) / 86400000),
    })),
  };
}

export async function generateTemporalSummary(businessId: string): Promise<TemporalState & { summary: string }> {
...
${state.ready_for_measurement.map((a) => {
  const days = Math.ceil((Date.now() - new Date(a.measurement_window_start).getTime()) / 86400000);
...
patient about? Plain text, no markdown.`;
  ...
  try {
    ...
    return { summary: ((result as { content: string } | null)?.content || '').trim(), ...state };
  } catch {
