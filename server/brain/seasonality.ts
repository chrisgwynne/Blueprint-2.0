 ...
 */
export async function detectSeasonalPatterns(businessId: string): Promise<number> {
  let learned = 0;
  ...
  for (const metric of METRICS_TO_LEARN) {
    ...
    `).all(businessId, metric) as Array<{ metric_value: string | number; recorded_at: string }>;
    ...
    for (const [dow, values] of Object.entries(dowBuckets)) {
      const avg = (values as number[]).reduce((s, v) => s + v, 0) / (values as number[]).length;
 ...
 */
export function isWithinSeasonalVariation(metric: string, changePct: number, businessId: string): boolean {
  const row = db.prepare(
    'SELECT pattern_data, confidence FROM seasonal_patterns WHERE business_id = ? AND metric_name = ? AND pattern_type = ?'
  ).get(businessId, metric, 'weekly') as { pattern_data: string; confidence: number } | null;
