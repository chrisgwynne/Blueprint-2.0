...
export async function extractIntent(
  ...
  conversationHistory: ConversationMessage[] | null | undefined,
  businessId: string
): Promise<Extraction[]> {
  ...
  ).all(businessId) as Array<{ title: string }>).map((g) => g.title);

  const history = (conversationHistory ?? []).slice(-5);
  const userMessage = `Business: ${business.name} (${(business.type as string | null) ?? 'general'})
...
Extract any intent with confidence >= 0.7. Empty array if nothing clear.`;
  ...
  try {
    ...
    const extractions = Array.isArray(parsed?.extractions) ? parsed!.extractions as Extraction[] : [];
    return extractions.filter((e) => e && e.type && e.type !== 'none' && ((e.confidence ?? 0) >= 0.7));
  } catch (err) {
    console.warn('[brain] extractIntent failed:', (err as Error).message);
    return [];
  }
}
 ...
 */
export async function actOnExtractions(
  extractions: Extraction[],
  businessId: string,
  conversationId?: string | null
): Promise<Record<string, unknown>[]> {
  const actions: Array<Record<string, unknown> | null> = [];
  ...
  for (const e of extractions) {
    try {
      ...
      console.warn('[brain] actOnExtraction failed:', (err as Error).message);
    }
  }

  return actions.filter(Boolean) as Record<string, unknown>[];
}

async function handleGoal(e: Extraction, businessId: string): Promise<Record<string, unknown> | null> {
  const s = e.structured ?? {};
  if (!s.title) return null;
  const id = crypto.randomUUID();
  const goalInsertParams: any[] = [
    id, businessId, s.title as string,
...
async function handleDecision(e: Extraction, businessId: string): Promise<Record<string, unknown> | null> {
  const s = e.structured ?? {};
  const decision = (s.decision as string | null) ?? e.raw_text?.slice(0, 120);
  ...
  try {
    ...
    const kbPath = (s.kb_path as string | null) || `decisions/decision-${today}-${crypto.randomUUID().slice(0, 6)}.md`;
    const body = `# Decision: ${decision}

**Date:** ${today}
**Affects:** ${(s.affects as string | null) ?? 'not specified'}
...
async function handleContext(e: Extraction, businessId: string): Promise<Record<string, unknown> | null> {
  const s = e.structured ?? {};
...
**When:** ${(s.date_hint as string | null) ?? 'not specified'}

## What this means for Blueprint

${(s.implications as string | null) ?? ''}
...
Agents should factor this into their analysis and avoid attributing metric changes to other causes during this period.`;
    ...
    await engine.writeFile(
      ...
      body,
      { title: (s.event as string).slice(0, 100), tags: ['context', 'upcoming'],
        created: today, updated: today, written_by: 'brain',
        review_status: 'auto_approved' },
      `context: ${(s.event as string).slice(0, 60)}`
    );
    return {
      type: 'context_noted',
...
async function handleScenario(e: Extraction, businessId: string): Promise<Record<string, unknown> | null> {
  const s = e.structured ?? {};
  const question = (s.question as string | null) || e.raw_text;
  if (!question) return null;
  try {
    const { modelScenarios } = await import('./scenario-engine.js') as unknown as { modelScenarios: (businessId: string, question: string, context: string) => Promise<Record<string, unknown> | null> };
    ...
    return {
      ...
      id: result.id,
      message: `I've modelled ${(result.scenarios as unknown[]).length} scenarios for "${question}".${rec} [View full scenario →](/scenarios?id=${result.id})`,
    };
  } catch (err) {
    console.warn('[brain] scenario from chat failed:', (err as Error).message);
    return null;
  }
}

async function handleInvestigation(e: Extraction, businessId: string): Promise<Record<string, unknown> | null> {
  const s = e.structured ?? {};
  const question = (s.question as string | null) || e.raw_text;
  if (!question) return null;
  try {
    const { runInvestigation } = await import('./investigation-engine.js') as unknown as { runInvestigation: (params: Record<string, unknown>) => Promise<Record<string, unknown> | null> };
    const result = await runInvestigation({
      businessId,
      metricName: (s.metric_hint as string | null) ?? null,
      question,
      triggeredBy: 'chat',
    });
    if (!result) return null;
    return {
      ...
      id: result.id,
      message: `I investigated: ${(result.plain_english as string | null) ?? '(see full report)'} [View full report →](/signals?investigation=${result.id})`,
    };
  } catch (err) {
    console.warn('[brain] investigation from chat failed:', (err as Error).message);
    return null;
  }
}

async function handleTask(e: Extraction, businessId: string): Promise<Record<string, unknown> | null> {
  const s = e.structured ?? {};
  if (!s.task_title) return null;
  try {
    const m = await import('../tasks/task-queue.js') as unknown as { createTask: (params: Record<string, unknown>) => Record<string, unknown> };
    const task = m.createTask({
      business_id: businessId,
      title: s.task_title,
      description: `Identified from conversation: "${e.raw_text}"`,
      proposed_by: 'brain',
      action_type: (s.action_type as string | null) || 'investigation',
      action_payload: {},
      trust_tier: 'yellow',
      priority: (s.urgency as string | null) || 'p3',
...
async function handleSignalImplied(
  e: Extraction,
  businessId: string,
  conversationId?: string | null
  ...
  try {
    const { createSignalIfNotDuplicate } = await import('../signals/signal-helpers.js') as unknown as {
      createSignalIfNotDuplicate: (params: Record<string, unknown>) => { id: string; created: boolean } | null;
    };
    ...
    const res = createSignalIfNotDuplicate({
      business_id: businessId,
      rule_id,
      type: ['anomaly', 'opportunity', 'risk', 'correlation'].includes(s.type as string) ? s.type : 'anomaly',
      severity: ['info', 'warning', 'alert', 'critical'].includes(s.severity as string) ? s.severity : 'info',
      title: s.title,
      description: (s.description as string | null) ?? `From conversation: "${e.raw_text}"`,
    ...
    console.warn('[brain] handleSignalImplied failed:', (err as Error).message);
    return null;
  }
}

async function handleAgentBriefNeeded(e: Extraction, businessId: string): Promise<Record<string, unknown> | null> {
  const s = e.structured ?? {};
  if (!s.to_agent || !s.brief) return null;

  // Only deliver to installed, active agents
  const agent = db.prepare("SELECT id FROM agents WHERE id = ? AND status = 'active'").get(s.to_agent as string) as { id: string } | null;
  if (!agent) return null;

  try {
    const { deliverAgentBrief } = await import('../agents/agent-inbox.js') as unknown as {
      deliverAgentBrief: (params: Record<string, unknown>) => Promise<{ delivered: boolean; triggered: boolean }>;
    };
    const priority = ['immediate', 'next_run', 'fyi'].includes(s.priority as string) ? (s.priority as string) : 'next_run';
    ...
    console.warn('[brain] handleAgentBriefNeeded failed:', (err as Error).message);
    return null;
  }
}

async function handleConnectorMention(e: Extraction, businessId: string): Promise<Record<string, unknown> | null> {
  const s = e.structured ?? {};
  if (!s.connector_name) return null;
  try {
    const { surfaceConnectorGap } = await import('../lib/connector-gap-handler.js') as unknown as {
      surfaceConnectorGap: (params: Record<string, unknown>) => Promise<{ first_time: boolean } | null>;
    };
    const res = await surfaceConnectorGap({
      business_id: businessId,
      connector_name: s.connector_name,
      source: 'chat',
      reason: (s.use_case as string | null) ?? `Mentioned in conversation: "${e.raw_text}"`,
    ...
    console.warn('[brain] handleConnectorMention failed:', (err as Error).message);
    return null;
  }
}
