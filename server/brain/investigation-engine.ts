...
interface Timeline {
  ...
  change_point: { at: string; before_avg: number; after_avg: number; delta: number } | null;
}

function gatherTimeline(businessId: string, metricName: string | null): Timeline {
  ...
  `).all(businessId, metricName) as TimelinePoint[];

  // Naive change-point: largest delta between consecutive 7-day averages
  let changePoint: Timeline['change_point'] = null;
  if (points.length >= 14) {
    let best = 0;
...
function gatherActionCorrelation(businessId: string, metricName: string | null): Record<string, unknown>[] {
...
function gatherMetricCorrelation(businessId: string, metricName: string | null): Record<string, unknown>[] {
  if (!metricName) return [];
  ...
  `).all(startOfWindow, businessId, metricName, startOfWindow) as Record<string, unknown>[]).filter((r) => {
    if (!r.prior_avg || r.prior_avg === 0) return false;
    const diff = Math.abs(((r.recent_avg as number) - (r.prior_avg as number)) / (r.prior_avg as number));
    return diff > 0.05;
  }).slice(0, 12);
}

async function gatherExternalContext(businessId: string, metricName: string | null): Promise<Array<{ path: string; title: string; excerpt: string }>> {
  try {
    ...
    const engine = result.engine as { listFiles: (dir: string) => Promise<string[]>; readFile: (f: string) => Promise<{ frontmatter?: Record<string, unknown>; content?: string }> };
    const signals = await engine.listFiles('signals').catch(() => [] as string[]);
    const picks: Array<{ path: string; title: string; excerpt: string }> = [];
    for (const f of signals.slice(-12)) {
      try {
        const parsed = await engine.readFile(f);
        picks.push({
          path: f,
          title: (parsed.frontmatter?.title as string) ?? f,
          excerpt: ((parsed.content || '') as string).slice(0, 220).replace(/\n+/g, ' ').trim(),
        });
      } catch {}
    }
    return picks;
...
function gatherSeasonalCheck(
  businessId: string,
  metricName: string | null,
  pointsTimeline: TimelinePoint[]
): { within_seasonal: boolean; change_pct: number } | null {
...
function gatherHistoricalPrecedent(businessId: string, metricName: string | null): Record<string, unknown>[] {
...
export async function runInvestigation({
  ...
  question = null,
  triggeredBy = 'human',
}: InvestigationParams): Promise<Record<string, unknown>> {
  ...
  if (signalId) {
    signal = db.prepare('SELECT * FROM signals WHERE id = ? AND business_id = ?').get(signalId, businessId) as Record<string, unknown> | null;
    if (signal?.data) {
      try {
        const data = JSON.parse(signal.data as string) as Record<string, unknown>;
        if (!metricName) metricName = (data.metric_name ?? data.metric ?? null) as string | null;
      } catch {}
    }
  }
  ...
  try {
    ...
    console.warn('[investigation] LLM failed:', (err as Error).message);
    recordAgentActivity({
      agentId: 'conductor', businessId, trigger: 'investigation',
      status: 'failed', reasoning: `Investigation failed: ${(err as Error).message.slice(0, 160)}`,
  ...
  `).run(
    ...
    (parsed.plain_english as string | null) ?? null,
    (parsed.primary_cause as string | null) ?? null,
    (parsed.primary_confidence as number | null) ?? null,
    (parsed.recommendation as string | null) ?? null,
    (parsed.recommended_action as string | null) ?? null,
    cacheUntil
  );
...
interface BuildPromptCtx {
  businessId: string;
  metricName: string | null;
  signal: Record<string, unknown> | null;
  question: string | null;
  timeline: Timeline;
  actionCorr: Record<string, unknown>[];
  metricCorr: Record<string, unknown>[];
  seasonal: { within_seasonal: boolean; change_pct: number } | null;
  external: Array<{ path: string; title: string; excerpt: string }>;
  historical: Record<string, unknown>[];
}

function buildPrompt(ctx: BuildPromptCtx): string {
  const L: string[] = [];
  L.push(`Business: ${ctx.businessId}`);
  if (ctx.metricName) L.push(`Metric under investigation: ${ctx.metricName}`);
  if (ctx.signal) L.push(`Signal: "${ctx.signal.title}" — ${(ctx.signal.description as string | null) ?? ''}`);
  ...
  if (ctx.timeline.points.length === 0) {
    ...
    L.push(`- ${ctx.timeline.points.length} data points, from ${(first.t as string).slice(0, 10)} (${first.v}) to ${(last.t as string).slice(0, 10)} (${last.v})`);
  ...
  for (const a of ctx.actionCorr) {
    const days = Math.ceil((Date.now() - new Date(a.measurement_window_start as string).getTime()) / 86400000);
    const inWindow = days >= (a.min_days as number) && days <= (a.expected_days as number) * 1.5;
  ...
  for (const m of ctx.metricCorr) {
    const diff = (((m.recent_avg as number) - (m.prior_avg as number)) / (m.prior_avg as number)) * 100;
    L.push(`- ${m.metric_name}: ${(m.prior_avg as number)?.toFixed(2)} → ${(m.recent_avg as number)?.toFixed(2)} (${diff > 0 ? '+' : ''}${diff.toFixed(1)}%)`);
  }
  L.push('');
  if (ctx.seasonal) {
  ...
  for (const h of ctx.historical) {
    L.push(`- "${h.title}" — ${h.verdict} ${h.change_pct != null ? `(${(h.change_pct as number) > 0 ? '+' : ''}${(h.change_pct as number).toFixed(1)}%)` : ''} on ${(h.check_date as string).slice(0, 10)}`);
  }
  L.push('');
...
function findCached(
  ...
  try { parsed = row.report_json ? JSON.parse(row.report_json as string) as Record<string, unknown> : {}; } catch {}
  return { id: row.id, ...parsed, created_at: row.created_at };
}

export function listInvestigations(businessId: string, limit = 50): Record<string, unknown>[] {
  return db.prepare(`
...
export function getInvestigation(id: string, businessId: string): Record<string, unknown> | null {
  const row = db.prepare('SELECT * FROM investigations WHERE id = ? AND business_id = ?').get(id, businessId) as Record<string, unknown> | null;
  if (!row) return null;
  let parsed: Record<string, unknown> = {};
  try { parsed = row.report_json ? JSON.parse(row.report_json as string) as Record<string, unknown> : {}; } catch {}
  return { ...row, ...parsed };
