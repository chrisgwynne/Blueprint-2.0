...
export function listCapabilities(businessId: string): Array<Record<string, unknown>> { syncCapabilitiesFromConnectors(businessId); return db.prepare('SELECT * FROM business_capabilities WHERE business_id = ? ORDER BY capability_key').all(businessId) as Array<Record<string, unknown>>; }
export function upsertCapability(businessId: string, data: Record<string, unknown>, actor = 'dashboard:human'): Record<string, unknown> {
  ...
  const after = db.prepare('SELECT * FROM business_capabilities WHERE business_id = ? AND capability_key = ?').get(businessId, key) as Record<string, unknown>;
  if (before?.status !== after.status) db.prepare("UPDATE applicability_suppressions SET status = 'cleared', cleared_at = CURRENT_TIMESTAMP WHERE business_id = ? AND capability_key = ? AND status = 'active'").run(businessId, key);
...
export function recordSuppression(input: { businessId: string; sourceType: string; sourceId?: string | null; candidateType: string; candidateKey: string; capabilityKey?: string | null; reason: string; evidenceStatus?: EvidenceStatus }): void {
  const exists = db.prepare("SELECT id FROM applicability_suppressions WHERE business_id = ? AND candidate_type = ? AND candidate_key = ? AND status = 'active'").get(input.businessId, input.candidateType, input.candidateKey);
  if (exists) return;
  db.prepare('INSERT INTO applicability_suppressions (id, business_id, source_type, source_id, candidate_type, candidate_key, capability_key, reason, evidence_status, reconsider_after, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)').run(generateId(), input.businessId, input.sourceType, input.sourceId ?? null, input.candidateType, input.candidateKey, input.capabilityKey ?? null, input.reason, input.evidenceStatus ?? 'observed', isoAfterDays(30));
}
...
export function createCorrection(input: Record<string, unknown>, actor = 'dashboard:human'): Record<string, unknown> {
  ...
  const previous = db.prepare('SELECT * FROM business_capabilities WHERE business_id = ? AND capability_key = ?').get(businessId, capabilityKey) as Record<string, unknown> | undefined;
  db.prepare(`INSERT INTO human_corrections (id, business_id, correction_type, assertion_key, previous_value, corrected_value, explanation, evidence_source, corrected_by, effective_at, confidence, permanence, review_at, affected_capability_id, suppression_behavior, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, businessId, String(input.correction_type ?? 'business_fact'), assertionKey, input.previous_value == null ? JSON.stringify(previous ?? null) : String(input.previous_value), String(input.corrected_value ?? ''), input.explanation ?? null, input.evidence_source ?? null, actor, input.effective_at ?? now, typeof input.confidence === 'number' ? input.confidence : 1, input.permanence ?? 'review_required', input.review_at ?? null, previous?.id ?? null, input.suppression_behavior ?? 'suppress_until_changed', requestedStatus, now);
  const correction = db.prepare('SELECT * FROM human_corrections WHERE id = ?').get(id) as Record<string, unknown>;
  ...
  db.prepare('UPDATE human_corrections SET affected_capability_id = ? WHERE id = ?').run(capability.id, id);
  ...
  const confirmed = db.prepare('SELECT * FROM human_corrections WHERE id = ?').get(id) as Record<string, unknown>;
  ...
  const add = (entityType: string, entityId: string, previous: string | null, next: string) => { const id = generateId(); db.prepare('INSERT INTO correction_impacts (id, correction_id, business_id, entity_type, entity_id, previous_status, new_status, reason, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)').run(id, correctionId, businessId, entityType, entityId, previous, next, reason); impacts.push({ id, entity_type: entityType, entity_id: entityId, previous_status: previous, new_status: next, reason }); };
  const like = `%${capabilityKey.replace(/_/g, '%')}%`;
  const signals = db.prepare(`SELECT id, status FROM signals WHERE business_id = ? AND (lower(title) LIKE lower(?) OR lower(description) LIKE lower(?) OR lower(data) LIKE lower(?)) AND COALESCE(lifecycle_status, status, 'open') NOT IN ('resolved','invalidated')`).all(businessId, like, like, like) as Array<{ id: string; status: string }>;
  ...
  const tasks = db.prepare(`SELECT id, status FROM tasks WHERE business_id = ? AND (lower(title) LIKE lower(?) OR lower(description) LIKE lower(?) OR lower(action_type) LIKE lower(?) OR lower(action_payload) LIKE lower(?)) AND status IN ('proposed','approved','deferred','manual_review')`).all(businessId, like, like, like, like) as Array<{ id: string; status: string }>;
  for (const t of tasks) { const next = t.status === 'approved' ? 'manual_review' : 'cancelled'; db.prepare('UPDATE tasks SET status = ?, applicability_status = ?, applicability_reason = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(next, 'not_applicable', reason, t.id); add('task', t.id, t.status, next); }
...
export function evaluateSignalLifecycle(businessId?: string): { evaluated: number; changed: number } {
  ...
  const rows = db.prepare(`SELECT s.id, s.business_id, s.connector_id, s.created_at, s.lifecycle_status, c.status AS connector_status, c.last_sync FROM signals s LEFT JOIN connectors c ON c.id = s.connector_id ${where}`).all(...params) as Array<Record<string, unknown>>;
  let changed = 0;
  for (const s of rows) {
    ...
    const newer = db.prepare(`SELECT id FROM signals WHERE business_id = ? AND rule_id = (SELECT rule_id FROM signals WHERE id = ?) AND id != ? AND created_at > ? ORDER BY created_at DESC LIMIT 1`).get(s.business_id, s.id, s.id, s.created_at) as { id: string } | undefined;
...
export function listRevenuePaths(businessId: string): Array<Record<string, unknown>> { return db.prepare('SELECT * FROM revenue_paths WHERE business_id = ? ORDER BY active DESC, role ASC, priority ASC, created_at DESC').all(businessId) as Array<Record<string, unknown>>; }
...
export function scheduleOutcomeMeasurements(taskId: string): number {
  ...
  for (const day of checkpoints) { const res = db.prepare(`INSERT OR IGNORE INTO outcome_measurement_runs (id, task_id, business_id, policy_id, checkpoint_at, checkpoint_day, baseline_value, state, evidence_status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending_measurement', 'unknown', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`).run(generateId(), taskId, task.business_id, policy.id ?? null, isoAfterDays(Number(day)), Number(day), task.target_metric_baseline ?? null); inserted += res.changes ?? 0; }
  return inserted;
}
export function evaluateDueOutcomeMeasurements(): { evaluated: number; changed: number } {
  const rows = db.prepare("SELECT * FROM outcome_measurement_runs WHERE state IN ('pending_measurement','measuring') AND checkpoint_at <= CURRENT_TIMESTAMP").all() as Array<Record<string, unknown>>; let changed = 0;
  for (const r of rows) { const state = r.baseline_value == null ? 'blocked_by_missing_data' : 'inconclusive'; const diagnostic = r.baseline_value == null ? 'No verified baseline value is available; not marking unsuccessful.' : 'Checkpoint reached but no connector observation is attached yet.'; db.prepare('UPDATE outcome_measurement_runs SET state = ?, verdict = ?, evidence_status = ?, diagnostic = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(state, state === 'blocked_by_missing_data' ? 'blocked' : 'inconclusive', r.baseline_value == null ? 'unknown' : 'observed', diagnostic, r.id); changed++; }
  return { evaluated: rows.length, changed };
}
export function recordRunEvent(runId: string, eventType: string, summary: string, opts: Record<string, unknown> = {}): void {
  const run = db.prepare('SELECT id, agent_id, business_id FROM agent_runs WHERE id = ?').get(runId) as Record<string, unknown> | undefined; if (!run) return;
  db.prepare(`INSERT INTO agent_run_events (id, run_id, business_id, agent_id, event_type, status, summary, request_id, correlation_id, related_resource_type, related_resource_id, duration_ms, error_category, metadata, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`).run(generateId(), runId, run.business_id, run.agent_id, eventType, opts.status ?? 'observed', summary, opts.request_id ?? null, opts.correlation_id ?? null, opts.related_resource_type ?? null, opts.related_resource_id ?? null, opts.duration_ms ?? null, opts.error_category ?? null, JSON.stringify(opts.metadata ?? {}));
}
export function requestRunCancellation(runId: string, actor = 'dashboard:human') {
  const row = db.prepare('SELECT * FROM agent_runs WHERE id = ?').get(runId) as Record<string, unknown> | undefined; if (!row) throw new Error('Run not found.');
  ...
  const res = db.prepare("UPDATE agent_runs SET status = 'cancellation_requested', cancellation_requested_at = CURRENT_TIMESTAMP, terminal_reason = 'Cancellation requested' WHERE id = ? AND status IN ('queued','running')").run(runId);
...
export function checkRunCancellation(runId: string): boolean { const row = db.prepare('SELECT status FROM agent_runs WHERE id = ?').get(runId) as { status: string } | undefined; return !!row && ['cancellation_requested', 'cancelling', 'cancelled'].includes(row.status); }
export function markRunCancelled(runId: string, reason = 'Stopped at a cooperative cancellation checkpoint.'): void { db.prepare("UPDATE agent_runs SET status = 'cancelled', cancellation_acknowledged_at = COALESCE(cancellation_acknowledged_at, CURRENT_TIMESTAMP), cancellation_completed_at = CURRENT_TIMESTAMP, completed_at = CURRENT_TIMESTAMP, terminal_reason = ? WHERE id = ? AND status IN ('running','cancellation_requested','cancelling')").run(reason, runId); recordRunEvent(runId, 'cancelled', reason, { status: 'cancelled' }); }
export function updateRunHeartbeat(runId: string, ttlMs = 120000): void { db.prepare('UPDATE agent_runs SET heartbeat_at = CURRENT_TIMESTAMP, heartbeat_expires_at = ? WHERE id = ?').run(new Date(Date.now() + ttlMs).toISOString(), runId); }
export function recordProviderPreflight(provider: string, model: string, status: 'passed' | 'failed' | 'blocked', evidence: Record<string, unknown>, ttlMs = 300000): Record<string, unknown> { const now = new Date().toISOString(); const expires = new Date(Date.now() + ttlMs).toISOString(); db.prepare(`INSERT INTO provider_preflight_cache (id, provider, model, status, evidence, checked_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(provider, model) DO UPDATE SET status=excluded.status, evidence=excluded.evidence, checked_at=excluded.checked_at, expires_at=excluded.expires_at`).run(generateId(), provider, model, status, JSON.stringify(evidence), now, expires); return { provider, model, status, evidence, checked_at: now, expires_at: expires }; }
export function getProviderPreflight(provider?: string, model?: string): Array<Record<string, unknown>> { if (provider && model) return db.prepare('SELECT * FROM provider_preflight_cache WHERE provider = ? AND model = ?').all(provider, model) as Array<Record<string, unknown>>; return db.prepare('SELECT * FROM provider_preflight_cache ORDER BY checked_at DESC LIMIT 100').all() as Array<Record<string, unknown>>; }
export function calculateScorecard(agentId: string, businessId?: string | null, actionCategory = 'all', days = 90): Record<string, unknown> {
  ...
  const runs = db.prepare(`SELECT status, cost_usd FROM agent_runs ar WHERE ar.agent_id = ? AND ar.started_at BETWEEN ? AND ? ${runBiz}`).all(...runParams) as Array<Record<string, unknown>>;
  ...
  const tasks = db.prepare(`SELECT status, confidence, action_type, applicability_status FROM tasks WHERE proposed_by = ? AND created_at BETWEEN ? AND ? ${taskBiz}`).all(...taskParams) as Array<Record<string, unknown>>;
  const outParams: unknown[] = [agentId]; if (businessId) outParams.push(businessId);
  const outcomes = db.prepare(`SELECT omr.state FROM outcome_measurement_runs omr JOIN tasks t ON t.id = omr.task_id WHERE t.proposed_by = ? ${businessId ? 'AND t.business_id = ?' : ''}`).all(...outParams) as Array<Record<string, unknown>>;
