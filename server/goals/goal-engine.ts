...
export async function checkAllGoals(businessId: string): Promise<number> {
  ...
  for (const g of goals) {
    ...
    catch (err: any) { console.warn(`[goals] check failed for ${g.id}:`, err.message); }
  }
  return checked;
}

export async function checkGoalById(goalId: string): Promise<any> {
  const goal = db.prepare('SELECT * FROM goals WHERE id=?').get(goalId) as GoalRow | null;
  if (!goal) throw new Error('Goal not found');
  return checkGoal(goal);
}

async function checkGoal(goal: GoalRow): Promise<any> {
  ...
  `).get(goal.business_id, goal.metric_name) as { metric_value: number } | null;
  ...
  const isMissed = deadline && daysLeft !== null && daysLeft < 0 && !isAchieved;
...
async function generateNote(goal: GoalRow, current: number, progressPct: number, daysLeft: number | null): Promise<string> {
  try {
...
async function fileGoalToKB(goal: GoalRow, outcome: string, note: string): Promise<void> {
  try {
