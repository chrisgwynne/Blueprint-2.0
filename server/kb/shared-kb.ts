...
let _engine: KBEngine | null = null;

export async function getSharedKB(): Promise<KBEngine | null> {
  ...
  try {
    ...
    console.warn('[shared-kb] init failed:', (err as Error).message);
    return null;
  }
}
 ...
 */
export async function readSharedKBForAgent(
  agentId: string,
  { limit = 6 } = {}
): Promise<{ path: string; title: string; summary: string; tags: unknown[] }[]> {
  try {
    const engine = await getSharedKB();
    if (!engine) return [];
    const allowedTags = AGENT_TAG_MAP[agentId] || ['all'];

    const picks: { path: string; title: string; summary: string; tags: unknown[] }[] = [];
      ...
      try {
        ...
        for (const f of files) {
          try {
            ...
            const tags = (parsed.frontmatter?.tags as unknown[]) || [];
            const match = tags.some((t) => allowedTags.includes(String(t).toLowerCase()));
            if (match || tags.length === 0) {
              picks.push({
                path: f,
                title: String(parsed.frontmatter?.title ?? f),
    ...
    console.warn('[shared-kb] read failed:', (err as Error).message);
    return [];
  }
}
 ...
 */
export async function getSharedKBStats(): Promise<{
  tactics: number;
  patterns: number;
  learnings: number;
  do_not_do: number;
 ...
 */
export async function maybeFileProvenTactic(
  task: {
    id: string;
    title: string;
...
`;
    ...
    console.warn('[shared-kb] proven tactic file failed:', (err as Error).message);
    return null;
  }
}
