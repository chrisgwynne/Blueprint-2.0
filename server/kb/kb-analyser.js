 ...
 * @param {object} [opts]
 * @param {number} [opts.hours=48]  — window of "recent" files
 * @param {boolean} [opts.force=false] — bypass the rate-limit guard
 * @returns {Promise<{signals, tasks, gaps, insights, contradictions, errors} | null>}
 */
export async function analyseKBForSignals(businessId, opts = {}) {
  ...
  const recent = (await kb.getRecentlyModified(hours)).filter(f =>
  ...
  try {
    ...
    console.warn(`[kb-analyser] LLM call failed for ${business.slug}:`, err.message);
    return { signals: 0, tasks: 0, gaps: 0, insights: 0, contradictions: 0, errors: [err.message] };
  }
  if (!analysis) {
...
function buildPrompt({ businessName, contents, goalsSummary, signalsSummary, tasksSummary }) {
...
function summariseActiveGoals(businessId) {
  try {
...
function summariseRecentSignals(businessId) {
  try {
    const rows = db.prepare(`
...
function summariseRecentTasks(businessId) {
  try {
    const rows = db.prepare(`
 ...
 */
export async function processAnalysis(analysis, { businessId, kb, businessSlug }) {
  const result = {
  ...
  for (const s of analysis.signals ?? []) {
    ...
    try {
      ...
      result.errors.push(`signal '${s.title}': ${err.message}`);
    }
  }
  ...
  for (const t of analysis.tasks ?? []) {
    ...
    try {
      ...
      result.errors.push(`task '${t.title}': ${err.message}`);
    }
  }
  ...
  for (const g of analysis.connector_gaps ?? []) {
    ...
    try {
      ...
      result.errors.push(`gap '${g.connector_name}': ${err.message}`);
    }
  }
  ...
  for (const c of analysis.contradictions ?? []) {
    ...
    try {
      ...
      result.errors.push(`contradiction: ${err.message}`);
    }
  }

  // Compounding insights — file back to KB
  for (const i of analysis.compounding_insights ?? []) {
    if (!i?.title || !i?.description || !(i.files?.length >= 2)) continue;
    try {
      ...
      result.errors.push(`insight '${i.title}': ${err.message}`);
    }
  }
...
export function scheduleKBAnalysis(businessId) {
  ...
  const timer = setTimeout(() => {
    ...
    analyseKBForSignals(businessId).catch(err => {
      console.warn(`[kb-analyser] Scheduled analysis failed for ${businessId}:`, err.message);
    });
  }, DEBOUNCE_MS);
 ...
 */
export function cancelScheduledAnalysis(businessId = null) {
  if (businessId) {
