 ...
 */
export function buildAgentROIContext(agentId: string, businessId: string): string {
 ...
 * raw number.
 */
export function buildConductorROIContext(businessId: string): string {
 ...
 */
export function buildReporterROIContext(businessId: string): string {
  if (!businessId) return '';

  const report = computeROIReport(businessId);
  if (!report) return '';

  const improvements = (report.attributed_improvements ?? []).slice(0, 5).map((r: any) =>
    `  - ${r.task_title} (${r.proposed_by}, ${r.metric_name}, ${r.change_pct?.toFixed?.(1)}%, ` +
    `£${r.estimated_monthly_usd}/mo, ${r.confidence} confidence)`
  );
  const declines = (report.attributed_declines ?? []).slice(0, 5).map((r: any) =>
    `  - ${r.task_title} (${r.proposed_by}, ${r.metric_name}, ${r.change_pct?.toFixed?.(1)}%, ` +
