...
function classifyConfidence(daysSinceInstall, outcomesCount) {
 ...
 */
export function computeROIReport(businessId, opts = {}) {
  if (!businessId) return null;

  const business = db.prepare('SELECT id, name, created_at FROM businesses WHERE id = ?').get(businessId);
  ...
  const attributedValueUsd = attributedImprovements.reduce(
    (sum, r) => sum + (Number.isFinite(r.estimated_monthly_usd) ? r.estimated_monthly_usd : 0), 0);
  const attributedDeclineUsd = attributedDeclines.reduce(
    (sum, r) => sum + (Number.isFinite(r.estimated_monthly_usd) ? Math.abs(r.estimated_monthly_usd) : 0), 0);
  ...
  const attributedMetrics = new Set(attributedImprovements.map(r => r.metric_name));
  const unattributedImprovements = observed
    .filter(o => o.direction === 'improved' && !attributedMetrics.has(o.metric_name))
    .map(o => ({
  ...
  const unattributedDeclines = observed
    .filter(o => o.direction === 'worsened' && !attributedMetrics.has(o.metric_name))
    .map(o => ({
      metric_name: o.metric_name,
      change_pct: o.change_pct,
      change_absolute: o.change_absolute,
  ...
  const costRow = db.prepare(`
    SELECT COALESCE(SUM(cost_usd), 0) as total
     ...
     WHERE business_id = ?
       AND date >= date(?)
       AND date <= date(?)
  `).get(businessId, periodStart, periodEnd);
...
function estimateNaturalTrajectory(businessId, periodStart, periodEnd, observed) {
  ...
  for (const metricName of REVENUE_METRICS) {
    const obs = observed.find(o => o.metric_name === metricName);
    if (!obs) continue;
    ...
    `).all(businessId, metricName, periodStart, periodStart);
    ...
    const projected = obs.baseline_value + slopePerDay * totalDays;
    projections.push({
...
function buildNarrative(ctx) {
  ...
  if (attributedImprovements.length === 0 && attributedDeclines.length === 0) {
    ...
    parts.push(
      ...
      `linked to Blueprint tasks — estimated +£${attributedValueUsd.toFixed(2)}/month. ` +
      `Spent $${totalCostUsd.toFixed(2)} on agent LLM calls over the period.${roiText}`
    );
  }
...
function metricFromVerdictDetail(detail) {
