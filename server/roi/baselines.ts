...
const BASELINE_METRICS_BY_CONNECTOR: Record<string, string[]> = {
  shopify: [
    'shopify.revenue_30d',
    'shopify.orders_30d',
    'shopify.aov',
...
export function recordBaseline({
...
}: RecordBaselineOpts): string | null {
  ...
  ).get(businessId, metricName) as { id: string } | null;
 ...
 */
export function captureBaselinesForConnector(businessId: string, connectorType: string): { recorded: number; skipped: number } {
  ...
  const biz = db.prepare('SELECT created_at FROM businesses WHERE id = ?').get(businessId) as { created_at: string } | null;
  ...
  for (const metricName of metrics) {
    const latest = db.prepare(`
      SELECT metric_value, recorded_at
       ...
       LIMIT 1
    `).get(businessId, metricName) as { metric_value: number; recorded_at: string } | null;
 ...
 */
export function getBaselines(businessId: string): BaselineRow[] {
 ...
 */
export function getBaseline(businessId: string, metricName: string): BaselineRow | null {
  if (!businessId || !metricName) return null;
  try {
    return db.prepare(
      'SELECT * FROM baselines WHERE business_id = ? AND metric_name = ?'
    ).get(businessId, metricName) as BaselineRow | null ?? null;
  } catch {
    return null;
  }
}
 ...
 */
export function getCurrentMetric(businessId: string, metricName: string): { value: number; at: string } | null {
  try {
    return db.prepare(`
      SELECT metric_value as value, recorded_at as at
       ...
       ORDER BY recorded_at DESC
       LIMIT 1
    `).get(businessId, metricName) as { value: number; at: string } | null ?? null;
  } catch {
    return null;
  }
}
