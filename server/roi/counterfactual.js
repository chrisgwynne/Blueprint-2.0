 ...
 */
export async function generateCounterfactual(taskId) {
  ...
  const baseline = target_metric_baseline
  ...
  if (directionAwayFromBaseline && Number.isFinite(gapEstimate.estimated_usd_per_month)) {
    ...
    monthlyCost = Math.abs(gapEstimate.estimated_usd_per_month);
    reasoning =
...
function writeRow(taskId, businessId, data) {
  const id = generateId();
  try {
    db.prepare(`
  ...
  try {
    const task = db.prepare('SELECT description FROM tasks WHERE id = ?').get(taskId);
  ...
  return db.prepare('SELECT * FROM counterfactual_estimates WHERE id = ?').get(id);
}
