 ...
 * @param {number} opts.baselineValue
 * @param {number} opts.currentValue
 ...
 * }}
 */
export function estimateMonthlyValue({ metricName, baselineValue, currentValue, businessId }) {
 ...
 */
function getAOV(businessId) {
  const direct = getBaseline(businessId, 'shopify.aov');
  if (direct && direct.baseline_value > 0) return direct.baseline_value;

  const rev = getBaseline(businessId, 'shopify.revenue_30d');
  const orders = getBaseline(businessId, 'shopify.orders_30d');
  if (rev?.baseline_value > 0 && orders?.baseline_value > 0) {
    return rev.baseline_value / orders.baseline_value;
  }
  return null;
}
 ...
 */
function getConversionRate(businessId) {
  ...
  if (shop && Number.isFinite(shop.baseline_value)) {
    ...
    return shop.baseline_value > 1 ? shop.baseline_value / 100 : shop.baseline_value;
  }
  const ga4 = getBaseline(businessId, 'ga4.conversion_rate');
  if (ga4 && Number.isFinite(ga4.baseline_value)) {
    return ga4.baseline_value > 1 ? ga4.baseline_value / 100 : ga4.baseline_value;
  }
  return null;
}
 ...
 */
function getMonthlyTraffic(businessId) {
  ...
  `).get(businessId);
  if (latest30?.v > 0) return latest30.v;

  const base30 = getBaseline(businessId, 'ga4.sessions_30d');
  if (base30?.baseline_value > 0) return base30.baseline_value;
  return null;
}

// ─── Per-metric estimators ────────────────────────────────────────────────────

function valueFromConversionRate(businessId, baseline, current, direction) {
...
function valueFromTraffic(businessId, baseline, current, direction, metricName) {
  const aov = getAOV(businessId);
...
function valueFromPageSpeed(businessId, metricName, baseline, current, direction) {
  const aov = getAOV(businessId);
  const traffic = getMonthlyTraffic(businessId);
  const cvr = getConversionRate(businessId);
...
function valueFromAvgPosition(businessId, baseline, current, direction) {
  const aov = getAOV(businessId);
  const cvr = getConversionRate(businessId);
  ...
  const deltaClicks = impressions.baseline_value * 0.03 * ctrMultiplier; // assume ~3% baseline CTR
  ...
  return {
    ...
    reasoning: `Avg position ${baseline.toFixed(1)} → ${current.toFixed(1)} ` +
               ...
               `× ~${(ctrMultiplier * 100).toFixed(0)}% CTR change on ${Math.round(impressions.baseline_value)} impressions ` +
    ...
    assumptions: [
      ...
      `Current impressions ~${Math.round(impressions.baseline_value)}.`,
    ],
  };
}

function valueFromGBP(businessId, metricName, baseline, current, direction) {
...
function partialKnowledge(label, direction, baseline, current, assumptions) {
  return {
    estimated_usd_per_month: null,
    confidence: 'unknown',
    direction,
