...
/**
 * @param {string} taskId
 * @param {{ verdict: string, changePct: number, detail: string, weeksAfter: number }} outcome
 */
export async function processTaskCompletion(taskId, outcome) {
  ...
  try {
    switch (verdict) {
      ...
      case 'no_change':
        ...
        break;
      case 'improved':
        // outcomes.js + shared-kb already handle the proven-tactic path.
    ...
    console.warn(`[task-intel] processing outcome for ${taskId} failed:`, err.message);
  }
}

// ─── Worsened → warning signal ───────────────────────────────────────────────

async function handleWorsened(task, outcome) {
  const { verdict, changePct, detail, weeksAfter } = outcome;
  const businessId = task.business_id;
...
async function handleNoChange(task, outcome) {
  ...
  `).get(businessId, followupTitle, task.id);
  if (existing) return;

  // 4-week check is more decisive than the 2-week check — only propose
  // follow-ups once the longer window has been measured. (outcomes.js runs
  ...
  try {
    ...
    console.warn('[task-intel] follow-up task creation failed:', err.message);
  }
}
