...
async function queryKBForContext(queryText, businessId) {
  try {
    ...
    for (const folder of ['signals', 'research', 'wiki']) {
      const files = await result.engine.listFiles(folder).catch(() => []);
...
async function getSeasonalContext(metricNames, businessId) {
 ...
 */
export async function assembleInvestigationContext(task, businessId) {
  const text = ((task.title ?? '') + ' ' + (task.description ?? '')).toLowerCase();

  // 1. Identify relevant connectors from task keywords
  const relevantConnectorTypes = new Set();
  ...
  const signal = task.signal_id
    ? db.prepare('SELECT * FROM signals WHERE id = ?').get(task.signal_id)
    : null;
