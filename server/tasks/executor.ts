...
function connectorTypeForAction(actionType: string | null): string | null {
  if (actionType === 'github_issue' || actionType === 'github_pr' || actionType === 'github_review_deploy') return 'github';
...
// ─── github_review_deploy handler ────────────────────────────────────────────
//
// SAFETY CONTRACT:
//  - If action_payload.head is present: creates a DRAFT PR for human review.
//    The PR is always draft=true; Blueprint never merges or deploys.
//  - If action_payload.head is absent: creates a GitHub issue containing
//    review/deploy instructions for a human to follow.
//  - In both paths, idempotency markers (execution-safety.ts) prevent
//    duplicates on crash-and-retry. This type is in DANGEROUS_ACTION_TYPES
//    so it is never auto-approved.

function buildReviewDeployBody(task: Task, marker: string, artifactType: 'issue' | 'pr'): string {
  const target = artifactType === 'pr' ? 'pull request' : 'issue';
  const lines: (string | null)[] = [
    '## Review & Deploy Request',
    '',
    `> **This is a human-review artifact.** Blueprint has NOT deployed anything.`,
    `> A human must review this ${target} and decide whether to merge/deploy.`,
    '',
    `**Proposed by:** ${task.proposed_by}`,
    `**Priority:** ${task.priority}`,
    task.confidence != null ? `**Confidence:** ${Math.round(task.confidence * 100)}%` : null,
  ...
  lines.push(
    ...
    '',
    `1. Review the changes described above.`,
    `2. Verify the changes are correct and safe to deploy.`,
    artifactType === 'pr'
      ? `3. Merge this pull request when satisfied, or close it to decline.`
      : `3. Apply the changes described and close this issue when complete, or close without action to decline.`,
    '',
    `_Created automatically by Blueprint on ${new Date().toISOString()}_`,
  );
  if (marker) lines.push('', `<!-- ${marker} -->`);
  return lines.filter((l): l is string => l !== null).join('\n');
}

async function executeGithubReviewDeploy(task: Task, job: ExecutionJobRow | null): Promise<ExecuteResult> {
  const payload = task.action_payload ?? {};
  const { credentials, config } = loadConnector(task.business_id, 'github');

  let owner = (payload.owner ?? credentials.owner ?? (config.owner as string | undefined)) as string | undefined;
  let repo = payload.repo as string | undefined;

  // `github_review_deploy` tasks generated by the review/deploy flow often
  // carry `repository: "owner/repo"` and `local_branch` / `base_branch`
  // rather than the lower-level github_pr payload shape. Accept that shape so
  // approved tasks do not strand execution jobs even though the safe outcome is
  // still only a draft PR or review issue.
  const repository = (payload.repository ?? payload.full_name) as string | undefined;
  ...
  if (!owner) throw new Error('GitHub owner is required (in task action_payload.repository/owner or connector config).');

  if (!repo) {
    const repos = (config.repos ?? credentials.repos) as string | undefined;
    if (repos) {
      const firstRepo = String(repos).split(',')[0]?.trim();
      repo = firstRepo?.includes('/') ? firstRepo.split('/').pop() : firstRepo;
    }
  }
  if (!repo) throw new Error('GitHub repo is required (in task action_payload.repository/repo or connector config.repos).');
  ...
  // Recovery path: job already has a reference (pre-crash persistence succeeded).
  if (existingRef?.pr_number) {
    return {
      outcome: `GitHub draft PR #${existingRef.pr_number} already created in ${existingRef.repo} (recovered from prior attempt)`,
      outcome_data: existingRef,
    };
  }
  if (existingRef?.issue_number) {
  ...
  const hasHead = (
    typeof payload.head === 'string' && (payload.head as string).length > 0
  ) || (
    typeof payload.remote_branch === 'string' && (payload.remote_branch as string).length > 0
  );
  const head = (payload.head ?? payload.remote_branch) as string | undefined;

  if (hasHead) {
    // Draft PR path — branch exists, create a draft PR for human review.
    // draft=true is unconditional: Blueprint never merges.
    const found = await (github as unknown as { searchByMarker: (c: unknown, o: string, r: string, m: string, t: 'issue' | 'pr') => Promise<{ number: number; html_url: string; state: string } | null> })
      .searchByMarker(credentials, owner, repo, marker, 'pr').catch(() => null);
    if (found?.number) {
      const recovered = { pr_number: found.number, pr_url: found.html_url, repo: `${owner}/${repo}`, state: found.state };
      return { outcome: `GitHub draft PR #${found.number} already exists in ${owner}/${repo} (verified via marker, not re-created)`, outcome_data: recovered };
    }

    const pr = await github.createPR(credentials, owner, repo, {
      title: (payload.title ?? task.title) as string,
      body:  buildReviewDeployBody(task, marker, 'pr'),
      head:  head as string,
      base:  ((payload.base ?? payload.base_branch) as string | undefined) ?? 'main',
      draft: true,
    }) as { number: number; html_url: string; state: string; draft: boolean; head?: { ref: string }; base?: { ref: string } } | null;
    ...
    return {
      outcome: `GitHub review draft PR #${pr.number} created in ${owner}/${repo} (human review required before merge/deploy)`,
      outcome_data: {
        pr_number: pr.number,
        pr_url: pr.html_url,
        repo: `${owner}/${repo}`,
    ...
    // Issue path — no branch, create a GitHub issue with instructions.
    const found = await (github as unknown as { searchByMarker: (c: unknown, o: string, r: string, m: string, t: 'issue' | 'pr') => Promise<{ number: number; html_url: string; state: string } | null> })
      .searchByMarker(credentials, owner, repo, marker, 'issue').catch(() => null);
    if (found?.number) {
      const recovered = { issue_number: found.number, issue_url: found.html_url, repo: `${owner}/${repo}`, state: found.state };
      return { outcome: `GitHub review issue #${found.number} already exists in ${owner}/${repo} (verified via marker, not re-created)`, outcome_data: recovered };
    }

    const issue = await github.createIssue(credentials, owner, repo, {
      title: (payload.title ?? task.title) as string,
      body:  buildReviewDeployBody(task, marker, 'issue'),
      // Do not invent Blueprint labels here: GitHub rejects unknown labels,
      // which would re-break the safe fallback path on repos without them.
      labels: (payload.labels as string[] | undefined) ?? [],
    }) as { number: number; html_url: string; state: string } | null;
    ...
    return {
      outcome: `GitHub review issue #${issue.number} created in ${owner}/${repo} (human review and deploy required)`,
      outcome_data: {
        issue_number: issue.number,
        issue_url: issue.html_url,
        repo: `${owner}/${repo}`,
...
export async function executeTask(taskId: string, job: ExecutionJobRow | null = null): Promise<{ ok: boolean; outcome?: string; outcome_data?: Record<string, unknown>; error?: string; status?: TaskStatus }> {
  ...
  try {
    switch (task.action_type) {
      ...
      case 'github_review_deploy':     result = await executeGithubReviewDeploy(task, job); break;
