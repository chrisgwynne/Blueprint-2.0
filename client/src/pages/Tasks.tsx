...
const PRIORITY_PILL: Record<number, string> = { 1: 'bp-pill-red', 2: 'bp-pill-amber', 3: 'bp-pill-blue', 4: 'bp-pill-grey' }
const PRIORITY_LABEL: Record<string | number, string> = { 1: 'P1', 2: 'P2', 3: 'P3', 4: 'P4', p1: 'P1', p2: 'P2', p3: 'P3', p4: 'P4' }
const PRIORITY_NUM: Record<string, number> = { p1: 1, p2: 2, p3: 3, p4: 4 }
...
function NewTaskModal({ onClose, onCreated, businessId }: NewTaskModalProps) {
...
function TaskListRow({ task, onApprove, onReject, onSelect }: TaskListRowProps) {
...
function DeferredBadge({ task }: { task: any }) {
  const [overriding, setOverriding] = useState(false)
  const days = task.deferred_until
    ? Math.max(0, Math.ceil((new Date(task.deferred_until).getTime() - Date.now()) / 86400000))
    : null
                    ...
                    <div style={{ marginTop: 8 }}>
                      ...
                      {Array.isArray(task.outcome_data.supporting_evidence) && task.outcome_data.supporting_evidence.length > 0 && (
                        <div style={{ marginBottom: 8, padding: '8px 10px', background: 'rgba(0,0,0,0.2)', borderRadius: 3 }}>
                          ...
                          {task.outcome_data.supporting_evidence.map((e: string, i: number) => (
...
function Tasks() {
      ...
      const list = Array.isArray(data)
        ...
        : Array.isArray((data as any)?.data)
          ...
          : Array.isArray((data as any)?.tasks)
            ...
            : []
      setTasks(list)
      setPendingTaskCount(list.filter((t: any) => t.status === 'proposed').length)
    } catch (err: any) {
  ...
  async function handleApprove(id: string, data: any) {
    try {
  ...
  async function handleReject(id: string, data: any) {
    try {
