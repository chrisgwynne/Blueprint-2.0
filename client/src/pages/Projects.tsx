...
function ProjectCard({ project, onOpen }: ProjectCardProps) {
  const days = project.target_date ? Math.ceil((new Date(project.target_date).getTime() - new Date().getTime()) / 86400000) : null
...
function NewProjectForm({ businessId, initial, onSaved, onCancel }: NewProjectFormProps) {
...
function ProjectDetail({ businessId, projectId, onBack }: ProjectDetailProps) {
  const [data, setData] = useState<ProjectDetailData | null>(null)

  useEffect(() => {
    getProject(businessId, projectId).then(setData as (v: unknown) => void).catch(() => {})
  ...
  return (
    <div>
      ...
      {([
        { key: 'workflows', title: 'Workflows', items: workflows, field: 'name' as const },
        { key: 'goals', title: 'Goals', items: goals, field: 'title' as const },
        { key: 'tasks', title: 'Tasks', items: tasks, field: 'title' as const },
        { key: 'signals', title: 'Signals', items: signals, field: 'title' as const },
        ...
        <div key={section.key} className="bp-card" style={{ padding: 16, marginBottom: 10 }}>
          ...
          {(section.items ?? []).slice(0, 10).map((item) => (
            <div key={item.id} style={{ padding: '6px 0', borderTop: '1px solid var(--bp-border)', fontFamily: 'var(--bp-font-mono)', fontSize: 11, color: 'var(--bp-text-2)' }}>
              {(item as Record<string, unknown>)[section.field] as React.ReactNode}
            </div>
          ))}
...
export default function Projects() {
  ...
  async function handleAskAI() {
    ...
    if (!context) return
    try {
      const result = await proposeProject(currentBusiness!.id, context) as { proposed?: Project & { name: string; description?: string } }
