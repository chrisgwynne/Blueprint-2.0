...
interface SoulFileEditorProps {
  agentId: string | undefined
  files: Record<string, string>
  onSaved?: () => void
}

function SoulFileEditor({ agentId, files, onSaved }: SoulFileEditorProps) {
...
function RunsTab({ agentId, businessId }: RunsTabProps) {
  ...
  const statusColor = (s: string) => s === 'complete' ? 'var(--bp-green)' : s === 'failed' ? 'var(--bp-red)' : 'var(--bp-amber)'
...
function MemoryTab({ agentId }: MemoryTabProps) {
  const addNotification = useStore(s => s.addNotification)
      ...
      {memory?.learnings?.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {[...memory.learnings].reverse().map((l: string, i: number) => (
...
function CalibrationTab({ agentId }: CalibrationTabProps) {
  const currentBusiness = useStore((s) => s.currentBusiness)
  const [data, setData] = useState<{ latest: any; history: any[] }>({ latest: null, history: [] })
  const [loading, setLoading] = useState(true)
...
function SettingsTab({ agentId, profile, onRefresh }: SettingsTabProps) {
  const addNotification = useStore(s => s.addNotification)
...
export default function AgentDetail() {
  const { agentId } = useParams<{ agentId: string }>() as { agentId: string }
  ...
  const load = useCallback(() => {
    Promise.all([
      ...
      setData(profileData)
      const a = (Array.isArray(agents) ? agents : []).find((x: any) => x.id === agentId)
      if (a) setAgent(a)
    }).finally(() => setLoading(false))
  }, [agentId])
