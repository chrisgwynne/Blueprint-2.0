...
function InsufficientState({ report }) {
...
function TrajectoryCharts({ trajectory, report }) {
...
function MetricChart({ series, baselineDate }) {
...
function AgentScorecards({ agents }) {
  if (!agents?.length) {
    return (
      <div style={{ padding: 16, color: 'var(--bp-text-3)', fontFamily: 'var(--bp-font-mono)', fontSize: 11 }}>
          ...
          {agents.map((a) => {
            ...
            return (
              <tr key={a.agent_id} style={{ borderBottom: '1px solid var(--bp-border)22' }}>
                ...
                <td style={{ padding: '8px', color: a.estimated_monthly_value_usd >= 0 ? 'var(--bp-green)' : 'var(--bp-red)' }}>
...
function HonestAssessment({ assessment, loading }) {
  if (loading) {
