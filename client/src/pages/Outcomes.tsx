...
interface AgentPerf {
  agent_id: string
  agent_name?: string
  tasks_proposed_30d?: number
  tasks_approved_30d?: number
...
function ImpactTimeline({ businessId }: ImpactTimelineProps) {
  const [metric, setMetric] = useState('gsc.total_clicks')
  const [data, setData] = useState<TimelineResponse | null>(null)

  useEffect(() => {
    if (!businessId) return
    getOutcomeTimeline(businessId, { metric, days: 90 }).then((d) => setData(d as TimelineResponse)).catch(() => setData(null))
  }, [businessId, metric])
            ...
            <ComposedChart data={chartData}>
              ...
              <Tooltip
                ...
                formatter={(val: unknown, name: string) => {
                  if (name === 'value') return [val as React.ReactNode, 'Metric']
                  return [val as React.ReactNode, name]
                }}
...
function AgentPerformance({ businessId }: AgentPerformanceProps) {
  const [agents, setAgents] = useState<AgentPerf[]>([])

  useEffect(() => {
    if (!businessId) return
    getAgentOutcomePerformance(businessId).then((a) => setAgents(a as AgentPerf[])).catch(() => setAgents([]))
  }, [businessId])
...
interface OutcomesListProps {
  outcomes: OutcomeItem[] | undefined
}

function OutcomesList({ outcomes }: OutcomesListProps) {
  if (!outcomes?.length) {
    return (
...
export default function Outcomes() {
  ...
  useEffect(() => {
    ...
    getOutcomes(currentBusiness.id, { verdict: verdictFilter, limit: 30 }).then((d) => setOutcomes(d as OutcomesResponse)).catch(() => setOutcomes(null))
