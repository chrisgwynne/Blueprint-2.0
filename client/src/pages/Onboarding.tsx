...
export default function Onboarding({ onComplete }: OnboardingProps) {
  ...
  const [psResult, setPsResult] = useState<ConnectorResult | null>(null)
  ...
  const [hireRecs, setHireRecs] = useState<HireRec[] | null>(null)   // null = not yet analysed
  const [hiring, setHiring] = useState<Record<string, 'pending' | 'done' | 'error'>>({})
  ...
  const [shopResult, setShopResult] = useState<ConnectorResult | null>(null)

  // Preload Google OAuth config on mount so we know whether to show setup.
  useEffect(() => {
    getGoogleOAuthConfig()
      .then((c) => setGoogleCfg(c as GoogleConfig))
      .catch(() => {})
  }, [])
  ...
  async function handleSaveGoogleCfg() {
    ...
    try {
      ...
      setGoogleCfg(updated as GoogleConfig)
      setGoogleClientSecret('')
      setShowGoogleSetup(false)
    } catch (err) {
      const e = err as Error
      alert('Failed to save Google OAuth: ' + e.message)
    } finally {
  ...
  async function handleShopifyConnect() {
    ...
    try {
      ...
      const e = err as Error
      setShopResult({ ok: false, error: e.message })
    } finally {
      setShopSaving(false)
    }
  }
  ...
  async function handlePageSpeedConnect() {
    ...
    setPsResult(null)
    try {
      const result = await testPageSpeed(psUrl) as { data: unknown }
      ...
      const e = err as Error
      setPsResult({ ok: false, error: e.message })
    } finally {
      setPsTesting(false)
    }
  }
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        try {
          const conns = await getConnectors(createdBiz.id) as Array<{ id: string; status: string }>
        ...
        const result = await getHireRecommendations(createdBiz.id) as { recommendations?: HireRec[] }
        if (cancelled) return
        setHireRecs(result?.recommendations ?? [])
      } catch (err) {
        const e = err as Error
        if (cancelled) return
        setHireError(e.message || 'Analysis failed')
        setHireRecs([])
      } finally {
  ...
  async function handleHireOne(templateId: string) {
    ...
    try {
      ...
      console.warn('[onboarding] hire failed:', e.message)
    }
  }
  ...
  async function handleFinish() {
    ...
    if (connectorAdded && createdBiz) {
      try {
        ...
        const connectors = await getConnectors(createdBiz.id) as Array<{ id: string; type: string }>
