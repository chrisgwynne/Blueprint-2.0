...
function parseData(signal: any): any {
...
function AnalysisBar({ run, onDismiss }: AnalysisBarProps) {
  if (!run) return null
...
function SummaryCard({ summary, onRunAnalysis, analysing }: SummaryCardProps) {
...
function CreateTaskPanel({ signal, onClose, onCreated }: CreateTaskPanelProps) {
...
function InvestigationModal({ open, onClose, signal, metric }: InvestigationModalProps) {
          ...
          <div>
            ...
            {Array.isArray(report.supporting_evidence) && report.supporting_evidence.length > 0 && (
              <div style={{ marginBottom: 14 }}>
                ...
                {report.supporting_evidence.map((e: string, i: number) => (
            ...
            {Array.isArray(report.alternative_causes) && report.alternative_causes.length > 0 && (
              <div style={{ marginBottom: 14 }}>
                ...
                {report.alternative_causes.map((a: any, i: number) => (
            ...
            {Array.isArray(report.what_rules_out) && report.what_rules_out.length > 0 && (
              <div style={{ marginBottom: 14 }}>
                ...
                {report.what_rules_out.map((r: string, i: number) => (
                  <div key={i} style={{ fontFamily: 'var(--bp-font-mono)', fontSize: 11, color: 'var(--bp-text-2)', padding: '3px 0' }}>
                    • {r}
                  </div>
                ))}
...
function AttributionBlock({ signal }: { signal: any }) {
  ...
  const total = a.attribution.reduce((s: number, x: any) => s + (x.probability || 0), 0) || 1
  ...
  return (
    <div style={{ marginBottom: 10, padding: 10, background: 'var(--bp-surface-2)', borderRadius: 4 }}>
      ...
      <div style={{ display: 'flex', height: 6, borderRadius: 3, overflow: 'hidden', marginBottom: 6 }}>
        {a.attribution.map((x: any, i: number) => (
...
function SignalCard({ signal, onUpdate }: SignalCardProps) {
  const addNotification = useStore((s) => s.addNotification)
  const [expanded, setExpanded]       = useState(false)
    ...
    <div style={{
      ...
      <div style={{ padding: '14px 18px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
          ...
          {data.connectors_involved?.length > 0 && data.connectors_involved.map((c: string) => (
            <span key={c} style={{
              fontFamily: 'var(--bp-font-mono)', fontSize: 9, padding: '1px 6px',
              borderRadius: 2, background: 'var(--bp-surface-3)',
              color: 'var(--bp-text-3)', textTransform: 'uppercase', letterSpacing: '0.06em',
...
function ClusterCard({ cluster, onDismiss, onUpdate }: ClusterCardProps) {
  const [acting, setActing] = useState(false)
...
function Signals() {
  ...
  const [filter,     setFilter]     = useState<{ severity: string; status: string; connector: string }>({ severity: 'all', status: 'open', connector: 'all' })
  ...
  const fetchSignals = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(res) ? res : ((res as any)?.data || (res as any)?.signals || [])
      setSignals(list)
    } catch (err) { console.error(err) }
  ...
  const fetchClusters = useCallback(async () => {
    ...
    try {
      ...
      setClusters(Array.isArray((res as any)?.clusters) ? (res as any).clusters : [])
    } catch { setClusters([]) }
  }, [currentBusiness])
  ...
  async function handleRunAnalysis() {
    ...
    setAnalysing(true)
    try {
      const { runId } = await triggerAnalysis(currentBusiness.id) as any
  ...
  function tabFilter(signal: any): boolean {
    switch (activeTab) {
