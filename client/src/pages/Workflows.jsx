...
function StatusBadge({ status }) {
  ...
  }[status] ?? { color: 'var(--bp-text-3)', label: status?.toUpperCase() ?? '' }
  return (
    <span style={{
...
// ─── Workflow card ────────────────────────────────────────────────────────────
function WorkflowCard({ workflow, onRun, onEdit, onDelete }) {
  ...
  const stepsPreview = workflow.steps?.map((s, i) => (
    <span key={s.index ?? i} style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
      ...
      {i < workflow.steps.length - 1 && <ArrowRight size={10} style={{ color: 'var(--bp-text-3)' }} />}
    </span>
  ))
...
// ─── Run card ─────────────────────────────────────────────────────────────────
function RunCard({ run, businessId, onRefresh }) {
  const [detail, setDetail] = useState(null)
  const [acting, setActing] = useState(null)

  useEffect(() => {
    getWorkflowRun(businessId, run.id).then(setDetail).catch(() => {})
  }, [businessId, run.id, run.status])

  async function handleApprove(stepIndex) {
    setActing(stepIndex)
    try {
  ...
  async function handleReject(stepIndex) {
...
function WorkflowEditor({ businessId, workflow, onSaved, onCancel }) {
...
// ─── Ask AI modal ─────────────────────────────────────────────────────────────
function AskAIModal({ businessId, onClose, onSaved }) {
  ...
  async function handleGenerate() {
    ...
    setGenerating(true)
    try {
      const result = await proposeWorkflow(businessId, text)
...
export default function Workflows() {
  ...
  const [editing, setEditing] = useState(null)  // workflow or {} for new
