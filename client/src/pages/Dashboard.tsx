...
function CardHeader({ title, children, link, linkLabel = 'View all' }: CardHeaderProps) {
  return (
...
function KeywordRow({ kw, maxImpressions }: { kw: Keyword; maxImpressions: number }) {
...
// Task status config
// ─────────────────────────────────────────────────────────
const TASK_PILL: Record<string, string> = { proposed: 'bp-pill-amber', approved: 'bp-pill-blue', executing: 'bp-pill-blue', complete: 'bp-pill-green', verified: 'bp-pill-grey', rejected: 'bp-pill-red' }
const TASK_BORDER: Record<string, string> = { proposed: 'var(--bp-amber)', approved: 'var(--bp-blue)', executing: 'var(--bp-blue)', complete: 'var(--bp-green)', verified: 'rgba(255,255,255,0.15)', rejected: 'var(--bp-red)' }
...
function TaskMiniCard({ task, onApprove, onReject }: TaskMiniCardProps) {
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        {task.confidence != null && (
          <span style={{ fontFamily: 'var(--bp-font-mono)', fontSize: 9, color: 'var(--bp-text-3)' }}>{Math.round((task.confidence as number) * 100)}%</span>
        )}
      </div>
...
function Dashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setDashboard(dash)
      const sigList: Signal[] = Array.isArray(sigs) ? sigs : (sigs?.data || sigs?.signals || [])
      setSignals(sigList)
      setOpenSignalCount(sigList.filter((s: Signal) => s.status === 'open' || !s.status).length)
      setPendingTaskCount((dash?.recent_tasks || []).filter((t: any) => t.status === 'proposed').length)
  ...
  async function handleAcknowledge(signal: Signal) {
    try {
  ...
  function spark(key: string) {
    ...
    return h.map((v: number, i: number) => ({ value: v, i }))
  }

  const trafficData = (() => {
    const sessions = dashboard?.history?.sessions || []
    if (sessions.length > 0) {
      return sessions.map((v: number, i: number) => ({ date: format(subDays(new Date(), sessions.length - 1 - i), 'MMM d'), sessions: v }))
    }
  ...
  const maxImpressions = keywords.reduce((m: number, k: any) => Math.max(m, k.impressions || 0), 1)
      ...
      <div style={{ marginBottom: 16 }}>
        <div className="bp-card" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          {loading ? <div style={{ padding: 16 }}><SectionSkeleton rows={4} /></div> : (
            <SignalFeed signals={signals.slice(0, 15) as any[]} onSignalClick={() => {}} onAcknowledge={handleAcknowledge as any} maxHeight={360} />
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        <div className="bp-card">
          ...
          {loading ? <div className="skeleton" style={{ height: 180, borderRadius: 4 }} /> :
           trafficData.some((d: any) => d.sessions) ? (
        ...
        <div className="bp-card">
          ...
          ) : keywords.map((kw: any, i: number) => <KeywordRow key={i} kw={kw} maxImpressions={maxImpressions} />)}
        </div>
      </div>
