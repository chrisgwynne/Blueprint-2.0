...
function ConversationsList({ businessId, active, onSelect, onNew, onPickAgent }) {
  const [conversations, setConversations] = useState([])
  const [agents, setAgents] = useState([])

  useEffect(() => {
    getConversations(businessId).then(setConversations).catch(() => setConversations([]))
...
function MentionDropdown({ query, agents, onPick, position }) {
...
export default function Chat() {
  ...
  useEffect(() => {
    getAgents().then(list => setAgents((list ?? []).filter(a => a.installed))).catch(() => {})
  }, [])
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const data = await getConversation(currentBusiness.id, activeId)
        setMessages(data.messages ?? [])
      } catch {}
    }, 3000)
  ...
  async function handleSend() {
    ...
    try {
      let convId = activeId
      if (!convId) {
        const created = await createConversation(currentBusiness.id, { title: text.slice(0, 60) })
        convId = created.id
        setActiveId(convId)
      ...
      const data = await getConversation(currentBusiness.id, convId)
      setConversation(data.conversation)
      setMessages(data.messages ?? [])
    } catch (err) {
  ...
  function handlePickAgent(agentId) {
    setInput(`@${agentId} `)
