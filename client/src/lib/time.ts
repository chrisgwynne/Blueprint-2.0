...
export function formatRelative(value: unknown, options = { addSuffix: true }): string {
  const d = parseTimestamp(value)
  if (!d) return '—'
  return _formatDistanceToNow(d, options)
}

export function formatDate(value: unknown, pattern = 'PP'): string {
  const d = parseTimestamp(value)
  if (!d) return '—'
  return _format(d, pattern)
}

export function formatDateTime(value: unknown, pattern = 'PP p'): string {
  const d = parseTimestamp(value)
  if (!d) return '—'
  return _format(d, pattern)
}
