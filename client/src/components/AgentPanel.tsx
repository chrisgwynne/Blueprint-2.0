  ...
  return (
    <div style={{
      ...
      <div style={{ fontFamily: 'var(--bp-font-mono)', fontSize: 9, color: 'var(--bp-text-3)', marginBottom: 6, paddingLeft: 20 }}>
        ...
        {(agent.cost_today_usd ?? 0) > 0 ? ` · $${agent.cost_today_usd!.toFixed(3)} today` : ''}
        {(agent.consecutive_failures ?? 0) > 0 ? ` · ⚠ ${agent.consecutive_failures} failures` : ''}
      </div>
...
export default function AgentPanel({ open, onToggle }: { open: boolean; onToggle: () => void }) {
