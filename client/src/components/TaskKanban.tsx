...
interface KanbanColumnData extends Column {
  tasks: Task[]
}
...
function KanbanColumn({ column, tasks, onUpdate, onSelect }: {
  ...
  // useSortable is used on cards; columns themselves are droppable areas
  const { setNodeRef, isOver } = { setNodeRef: null as any, isOver: false }
...
function TaskKanban({ tasks = [], onUpdate, onSelect }: {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    const targetCol = COLUMNS.find(c => columns.find(col => col.id === c.id)?.tasks.some(t => t.id === over.id))
      || COLUMNS.find(c => c.id === over.id)

    if (targetCol) {
      try {
        await updateTask(active.id as string, { status: targetCol.id })
        onUpdate?.()
      } catch (err: any) {
        addNotification({ type: 'error', message: err.message })
      }
