...
function unwrapMetric(v: unknown): unknown {
  if (v == null) return v
  if (typeof v !== 'object') return v
  const obj = v as MetricWrapper
...
interface MetricCardProps {
  ...
  iconColor?: string
  loading?: boolean
  invertPolarity?: boolean  // true = lower is better (avg position)
  accentColor?: string      // override sparkline/trend colour
}
...
function MetricCard({
  ...
  const trendValue = trend ?? (
    prev !== undefined && prev !== null && prev !== 0 && value !== undefined && value !== null
      ? Math.round((((value as number) - (prev as number)) / Math.abs(prev as number)) * 100)
      : null
  )
