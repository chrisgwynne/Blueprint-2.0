...
// Business Switcher
// ============================================
function BusinessSwitcher({ collapsed }) {
  ...
  const logo = currentBusiness.settings?.logo
        ...
        <>
          ...
          <div style={{
            ...
            {businesses.map((biz) => (
              ...
              >
                {biz.settings?.logo ? (
                  <img src={biz.settings.logo} alt="" style={{ width: 18, height: 18, borderRadius: 3, objectFit: 'cover' }} />
                ) : (
  ...
  useEffect(() => {
    ...
    getConnectors(currentBusiness.id)
      .then((data) => {
        ...
        setConnectors(list.filter((c) => c.status === 'connected' || c.status === 'syncing'))
      })
      .catch(() => {})
  }, [currentBusiness])
